...
{
    RLSetTraceLogLevel(RL_E_LOG_NONE);
    ...
    if (argc > 1)
    {
        ...
        if (strcmp(argv[1], "create") == 0)
        {
            ...
            else
            {
                ...
                int catIndex = RLTextFindIndex(argv[2], "_");
                if (catIndex > 3)
                {
                    char cat[12] = { 0 };
                    ...
                    {
                        if (RLTextIsEqual(cat, exCategories[i])) { catFound = true; break; }
                    }

                    if (catFound)
                    {
                        strcpy(exName, argv[2]); // Register filename for new example creation
                        strncpy(exCategory, exName, RLTextFindIndex(exName, "_"));
                        opCode = OP_CREATE;
                    }
        ...
        else if (strcmp(argv[1], "add") == 0)
        {
            ...
            {
                if (RLIsFileExtension(argv[2], ".c")) // Check for valid file extension: input
                {
                    if (RLFileExists(argv[2]))
                    {
                        // Security checks for file name to verify category is included
                        int catIndex = RLTextFindIndex(RLGetFileName(argv[2]), "_");
                        if (catIndex > 3)
                        {
                            char cat[12] = { 0 };
                            strncpy(cat, RLGetFileName(argv[2]), catIndex);
                            bool catFound = false;
                            for (int i = 0; i < REXM_MAX_EXAMPLE_CATEGORIES; i++)
                            {
                                if (RLTextIsEqual(cat, exCategories[i])) { catFound = true; break; }
                            ...
                            if (catFound)
                            {
                                ...
                                strcpy(exName, RLGetFileNameWithoutExt(inFileName)); // Register example name
                                strncpy(exCategory, exName, RLTextFindIndex(exName, "_"));
                                opCode = OP_ADD;
                            }
                            else LOG("WARNING: Example category is not valid\n");
        ...
        else if (strcmp(argv[1], "rename") == 0)
        {
            ...
            else
            {
                ...
                char *exColInfo = RLLoadFileText(exCollectionFilePath);
                if (RLTextFindIndex(exColInfo, argv[2]) != -1) // Example in the collection
                {
                    // Security checks for new file name to verify category is included
                    int newCatIndex = RLTextFindIndex(argv[3], "_");
                    if (newCatIndex > 3)
                    {
                        char cat[12] = { 0 };
                        ...
                        for (int i = 0; i < REXM_MAX_EXAMPLE_CATEGORIES; i++)
                        {
                            if (RLTextIsEqual(cat, exCategories[i])) { newCatFound = true; break; }
                        }
                        ...
                        if (newCatFound)
                        {
                            ...
                            strncpy(exRecategory, exRename, RLTextFindIndex(exRename, "_"));
                            opCode = OP_RENAME;
                        }
        ...
        else if ((strcmp(argv[1], "build") == 0) || (strcmp(argv[1], "test") == 0) || (strcmp(argv[1], "testlog") == 0))
        {
            ...
            else
            {
                ...
                {
                    if (!RLTextIsEqual(exBuildListInfo[i].category, "others"))
                    {
    ...
    switch (opCode)
    {
        case OP_CREATE:     // Create: New example from template
        {
            ...
            char *exText = RLLoadFileText(exTemplateFilePath);
            char *exTextUpdated[6] = { 0 };
            int exIndex = RLTextFindIndex(exText, "/****************");

            // Update required info with some defaults
            exTextUpdated[0] = RLTextReplace(exText + exIndex, "<module>", exCategory);
            exTextUpdated[1] = RLTextReplace(exTextUpdated[0], "<name>", exName + strlen(exCategory) + 1);
            ...
            RLSaveFileText(RLTextFormat("%s/%s/%s.c", exBasePath, exCategory, exName), exTextUpdated[1]);
            for (int i = 0; i < 6; i++) { RLMemFree(exTextUpdated[i]); exTextUpdated[i] = NULL; }
            RLUnloadFileText(exText);
        }
        case OP_ADD:     // Add: Example from command-line input filename
        {
            ...
            if (opCode != OP_CREATE) RLFileCopy(inFileName, RLTextFormat("%s/%s/%s.c", exBasePath, exCategory, exName));

            // Create: raylib/examples/<category>/<category>_example_name.png
            if (RLFileExists(RLTextFormat("%s/%s.png", RLGetDirectoryPath(inFileName), exName)))
            {
                LOG("INFO: [%s] Copying file screenshot...\n", RLGetFileName(inFileName));
                RLFileCopy(RLTextFormat("%s/%s.png", RLGetDirectoryPath(inFileName), exName),
                    RLTextFormat("%s/%s/%s.png", exBasePath, exCategory, exName));
            }
            else // No screenshot available next to source file
            {
                ...
                RLFileCopy(exTemplateScreenshot, RLTextFormat("%s/%s/%s.png", exBasePath, exCategory, exName));
            }
            ...
            char **resPaths = LoadExampleResourcePaths(RLTextFormat("%s/%s.c", RLGetDirectoryPath(inFileName), exName), &resPathCount);

            if (resPathCount > 0)
            {
                LOG("INFO: [%s] Required resources found: %i\n", RLGetFileName(inFileName), resPathCount);
                ...
                for (int r = 0; r < resPathCount; r++)
                {
                    ...
                    if (RLTextFindIndex(resPaths[r], "glsl%i") > -1)
                    {
                        int glslVer[3] = { 100, 120, 330 };

                        for (int v = 0; v < 3; v++)
                        {
                            char *resPathUpdated = RLTextReplace(resPaths[r], "glsl%i", RLTextFormat("glsl%i", glslVer[v]));

                            LOG("INFO: [%s] Resource required [%i/%i]: %s\n", RLGetFileName(inFileName), r, resPathCount, resPathUpdated);

                            if (RLFileExists(RLTextFormat("%s/%s", RLGetDirectoryPath(inFileName), resPathUpdated)))
                            {
                                // Verify the resources are placed in "resources" directory
                                if (RLTextFindIndex(resPathUpdated, "resources/") > 0)
                                {
                                    ...
                                    RLFileCopy(RLTextFormat("%s/%s", RLGetDirectoryPath(inFileName), resPathUpdated),
                                        RLTextFormat("%s/%s/%s", exBasePath, exCategory, resPathUpdated));
                                }
                                else LOG("WARNING: Example resource must be placed in 'resources' directory next to .c file\n");
                            }
                            else LOG("WARNING: Example resource can not be found in: %s\n", RLTextFormat("%s/%s", RLGetDirectoryPath(inFileName), resPathUpdated));
                    ...
                    {
                        LOG("INFO: [%s] Resource required [%i/%i]: %s\n", RLGetFileName(inFileName), r, resPathCount, resPaths[r]);

                        if (RLFileExists(RLTextFormat("%s/%s", RLGetDirectoryPath(inFileName), resPaths[r])))
                        {
                            // Verify the resources are placed in "resources" directory
                            if (RLTextFindIndex(resPaths[r], "resources/") > 0)
                            {
                                // NOTE: Look for resources in the path of the provided .c to be added
                                // To be copied to <category>/resources directory, extra dirs are automatically created if required
                                RLFileCopy(RLTextFormat("%s/%s", RLGetDirectoryPath(inFileName), resPaths[r]),
                                    RLTextFormat("%s/%s/%s", exBasePath, exCategory, resPaths[r]));
                            }
                            else LOG("WARNING: Example resource must be placed in 'resources' directory next to .c file\n");
                        }
                        else LOG("WARNING: Example resource can not be found in: %s\n", RLTextFormat("%s/%s", RLGetDirectoryPath(inFileName), resPaths[r]));
                    }
                }
            }
            ...
            //------------------------------------------------------------------------------------------------
            char *exCollectionList = RLLoadFileText(exCollectionFilePath);
            if (RLTextFindIndex(exCollectionList, exName) == -1) // Example not found
            {
                LOG("INFO: [%s] Adding example to collection list (%s)\n", RLGetFileName(inFileName), exCategory);
                ...
                if (RLTextIsEqual(exCategory, "core")) nextCategoryIndex = 1;
                else if (RLTextIsEqual(exCategory, "shapes")) nextCategoryIndex = 2;
                else if (RLTextIsEqual(exCategory, "textures")) nextCategoryIndex = 3;
                else if (RLTextIsEqual(exCategory, "text")) nextCategoryIndex = 4;
                else if (RLTextIsEqual(exCategory, "models")) nextCategoryIndex = 5;
                else if (RLTextIsEqual(exCategory, "shaders")) nextCategoryIndex = 6;
                else if (RLTextIsEqual(exCategory, "audio")) nextCategoryIndex = 7;
                else if (RLTextIsEqual(exCategory, "others")) nextCategoryIndex = -1; // Add to EOF

                // Get required example info from example file header (if provided)

                // NOTE: Load example info from provided example header
                rlExampleInfo *exInfo = LoadExampleInfo(RLTextFormat("%s/%s/%s.c", exBasePath, exCategory, exName));
                ...
                if (nextCategoryIndex == -1)
                {
                    ...
                    sprintf(exCollectionListUpdated + endIndex, RLTextFormat("%s;%s;%s;%s;%s;%i;%i;\"%s\";@%s\n",
                ...
                else
                {
                    ...
                    int categoryIndex = RLTextFindIndex(exCollectionList, exCategories[nextCategoryIndex]);
                    memcpy(exCollectionListUpdated, exCollectionList, categoryIndex);
                    int textWritenSize = sprintf(exCollectionListUpdated + categoryIndex, RLTextFormat("%s;%s;%s;%s;%s;%i;%i;\"%s\";@%s\n",
                        exInfo->category, exInfo->name, starsText, exInfo->verCreated, exInfo->verUpdated, exInfo->yearCreated, exInfo->yearReviewed, exInfo->author, exInfo->authorGitHub));
                ...
                RLSaveFileText(exCollectionFilePath, exCollectionListUpdated);
                RL_FREE(exCollectionListUpdated);
            }
            else LOG("WARNING: ADD: Example is already on the collection\n");
            RLUnloadFileText(exCollectionList);
            //------------------------------------------------------------------------------------------------
            ...
            //------------------------------------------------------------------------------------------------
            LOG("INFO: [%s] Creating example project\n", RLTextFormat("%s/../projects/VS2022/examples/%s.vcxproj", exBasePath, exName));
            // WARNING: When adding new project a unique UUID should be assigned!
            RLFileCopy(RLTextFormat("%s/../projects/VS2022/examples/core_basic_window.vcxproj", exBasePath),
                RLTextFormat("%s/../projects/VS2022/examples/%s.vcxproj", exBasePath, exName));
            RLFileTextReplace(RLTextFormat("%s/../projects/VS2022/examples/%s.vcxproj", exBasePath, exName),
                "core_basic_window", exName);
            RLFileTextReplace(RLTextFormat("%s/../projects/VS2022/examples/%s.vcxproj", exBasePath, exName),
                "..\\..\\examples\\core", RLTextFormat("..\\..\\examples\\%s", exCategory));
            ...
            AddVSProjectToSolution(exVSProjectSolutionFile,
                RLTextFormat("%s/../projects/VS2022/examples/%s.vcxproj", exBasePath, exName), exCategory);
            //------------------------------------------------------------------------------------------------
...
#if defined(_WIN32)
            LOG("INFO: [%s] Building example for PLATFORM_WEB (Host: Win32)\n", RLGetFileNameWithoutExt(inFileName));
            //putenv("RAYLIB_DIR=C:\\GitHub\\raylib");
            _putenv("PATH=%PATH%;C:\\raylib\\w64devkit\\bin");
            system(RLTextFormat("mingw32-make -C %s -f Makefile.Web %s/%s PLATFORM=PLATFORM_WEB -B", exBasePath, exCategory, exName));
#else
            LOG("INFO: [%s] Building example for PLATFORM_WEB (Host: POSIX)\n", RLGetFileNameWithoutExt(inFileName));
            system(RLTextFormat("make -C %s -f Makefile.Web %s/%s PLATFORM=PLATFORM_WEB -B", exBasePath, exCategory, exName));
#endif
            // Update generated .html metadata
            LOG("INFO: [%s] Updating HTML Metadata...\n", RLTextFormat("%s.html", exName));
            UpdateWebMetadata(RLTextFormat("%s/%s/%s.html", exBasePath, exCategory, exName),
                RLTextFormat("%s/%s/%s.c", exBasePath, exCategory, exName));

            // Copy results to web side
            LOG("INFO: [%s] Copy example build to raylib.com\n", exName);
            RLFileCopy(RLTextFormat("%s/%s/%s.html", exBasePath, exCategory, exName),
                RLTextFormat("%s/%s/%s.html", exWebPath, exCategory, exName));
            RLFileCopy(RLTextFormat("%s/%s/%s.data", exBasePath, exCategory, exName),
                RLTextFormat("%s/%s/%s.data", exWebPath, exCategory, exName));
            RLFileCopy(RLTextFormat("%s/%s/%s.wasm", exBasePath, exCategory, exName),
                RLTextFormat("%s/%s/%s.wasm", exWebPath, exCategory, exName));
            RLFileCopy(RLTextFormat("%s/%s/%s.js", exBasePath, exCategory, exName),
                RLTextFormat("%s/%s/%s.js", exWebPath, exCategory, exName));
            //------------------------------------------------------------------------------------------------
        ...
        case OP_RENAME:     // Rename
        {
            ...
            if (strcmp(exCategory, exRecategory) == 0)
            {
                ...
                RLFileTextReplace(exCollectionFilePath, RLTextFormat("%s;%s", exCategory, exName),
                    RLTextFormat("%s;%s", exRecategory, exRename));

                // Edit: Rename example code and screenshot files .c and .png
                RLFileRename(RLTextFormat("%s/%s/%s.c", exBasePath, exCategory, exName),
                    RLTextFormat("%s/%s/%s.c", exBasePath, exCategory, exRename));
                RLFileRename(RLTextFormat("%s/%s/%s.png", exBasePath, exCategory, exName),
                    RLTextFormat("%s/%s/%s.png", exBasePath, exCategory, exRename));
                ...
                for (int i = 0; i < exListCount; i++)
                {
                    if (strcmp(exList[i].name, exRename) == 0)
                        UpdateSourceMetadata(RLTextFormat("%s/%s/%s.c", exBasePath, exCategory, exRename), &exList[i]);
                }
                ...
                RLFileTextReplace(RLTextFormat("%s/Makefile", exBasePath), exName, exRename);
                RLFileTextReplace(RLTextFormat("%s/Makefile.Web", exBasePath), exName, exRename);
                RLFileTextReplace(RLTextFormat("%s/README.md", exBasePath), exName, exRename);
                RLFileTextReplace(RLTextFormat("%s/../common/examples.js", exWebPath),
                    exName + strlen(exCategory) + 1, exRename + strlen(exRecategory) + 1); // Skip category

                // Edit: Rename example project and solution
                RLFileTextReplace(RLTextFormat("%s/../projects/VS2022/examples/%s.vcxproj", exBasePath, exName), exName, exRename);
                RLFileRename(RLTextFormat("%s/../projects/VS2022/examples/%s.vcxproj", exBasePath, exName),
                    RLTextFormat("%s/../projects/VS2022/examples/%s.vcxproj", exBasePath, exRename));
                RLFileTextReplace(RLTextFormat("%s/../projects/VS2022/raylib.sln", exBasePath), exName, exRename);
            }
            else
            {
                ...
                RLFileCopy(RLTextFormat("%s/%s/%s.c", exBasePath, exCategory, exName),
                    RLTextFormat("%s/%s/%s.c", exBasePath, exCategory, exRename));
                RLFileRemove(RLTextFormat("%s/%s/%s.c", exBasePath, exCategory, exName));
                // Edit: Rename example screenshot file (copy and remove)
                RLFileCopy(RLTextFormat("%s/%s/%s.png", exBasePath, exCategory, exName),
                    RLTextFormat("%s/%s/%s.png", exBasePath, exCategory, exRename));
                RLFileRemove(RLTextFormat("%s/%s/%s.png", exBasePath, exCategory, exName));
            ...
            RLFileRemove(RLTextFormat("%s/%s/%s.html", exWebPath, exCategory, exName));
            RLFileRemove(RLTextFormat("%s/%s/%s.data", exWebPath, exCategory, exName));
            RLFileRemove(RLTextFormat("%s/%s/%s.wasm", exWebPath, exCategory, exName));
            RLFileRemove(RLTextFormat("%s/%s/%s.js", exWebPath, exCategory, exName));

            // Recompile example (on raylib side)
            // WARNING: EMSDK_PATH must be set to proper location when calling from GitHub Actions
#if defined(_WIN32)
            _putenv("PATH=%PATH%;C:\\raylib\\w64devkit\\bin");
            system(RLTextFormat("mingw32-make -C %s -f Makefile.Web %s/%s PLATFORM=PLATFORM_WEB -B", exBasePath, exRecategory, exRename));
#else
            system(RLTextFormat("make -C %s -f Makefile.Web %s/%s PLATFORM=PLATFORM_WEB -B", exBasePath, exRecategory, exRename));
#endif
            // Update generated .html metadata
            UpdateWebMetadata(RLTextFormat("%s/%s/%s.html", exBasePath, exCategory, exRename),
                RLTextFormat("%s/%s/%s.c", exBasePath, exCategory, exRename));

            // Copy results to web side
            RLFileCopy(RLTextFormat("%s/%s/%s.html", exBasePath, exRecategory, exRename),
                RLTextFormat("%s/%s/%s.html", exWebPath, exRecategory, exRename));
            RLFileCopy(RLTextFormat("%s/%s/%s.data", exBasePath, exRecategory, exRename),
                RLTextFormat("%s/%s/%s.data", exWebPath, exRecategory, exRename));
            RLFileCopy(RLTextFormat("%s/%s/%s.wasm", exBasePath, exRecategory, exRename),
                RLTextFormat("%s/%s/%s.wasm", exWebPath, exRecategory, exRename));
            RLFileCopy(RLTextFormat("%s/%s/%s.js", exBasePath, exRecategory, exRename),
                RLTextFormat("%s/%s/%s.js", exWebPath, exRecategory, exRename));

#if defined(RENAME_AUTO_COMMIT_CREATION)
            // Create GitHub commit with changes (local)
            putenv("PATH=%PATH%;C:\\Program Files\\Git\\bin");
            RLChangeDirectory("C:\\GitHub\\raylib");
            system("git --version");
            system("git status");
            system("git add -A");
            int result = system(RLTextFormat("git commit -m \"REXM: RENAME: example: `%s` --> `%s`\"", exName, exRename)); // Commit changes (only tracked files)
            if (result != 0) LOG("WARNING: Error committing changes\n");
            RLChangeDirectory("C:/GitHub/raylib.com");
            system("git add -A");
            result = system(RLTextFormat("git commit -m \"REXM: RENAME: example: `%s` --> `%s`\"", exName, exRename)); // Commit changes (only tracked files)
            if (result != 0) LOG("WARNING: Error committing changes\n");
        ...
        case OP_REMOVE:     // Remove
        {
            ...
            char *exCollectionList = RLLoadFileText(exCollectionFilePath);
            int exIndex = RLTextFindIndex(exCollectionList, RLTextFormat("%s;%s", exCategory, exName));
            ...
            */
            ...
            LOG("INFO: [%s] Removing example project file\n", RLTextFormat("%s.vcxproj", exName));
            RLFileRemove(RLTextFormat("%s/../projects/VS2022/examples/%s.vcxproj", exBasePath, exName));

            // Edit: raylib/projects/VS2022/raylib.sln --> Remove example project
            LOG("INFO: [%s] Removing example from raylib solution (.sln)\n", exName);
            RemoveVSProjectFromSolution(RLTextFormat("%s/../projects/VS2022/raylib.sln", exBasePath), exName);

            // Remove: Delete example build from local copy (if exists)
            RLFileRemove(RLTextFormat("%s/%s/%s.html", exBasePath, exCategory, exName));
            RLFileRemove(RLTextFormat("%s/%s/%s.data", exBasePath, exCategory, exName));
            RLFileRemove(RLTextFormat("%s/%s/%s.wasm", exBasePath, exCategory, exName));
            RLFileRemove(RLTextFormat("%s/%s/%s.js", exBasePath, exCategory, exName));
        ...
        case OP_BUILD:
        {
            ...
            for (int i = 0; i < exBuildListCount; i++)
            {
                ...
                system(RLTextFormat("mingw32-make -C %s %s/%s PLATFORM=PLATFORM_DESKTOP -B", exBasePath, exCategory, exName));
#elif defined(PLATFORM_DRM)
                LOG("INFO: [%s] Building example for PLATFORM_DRM (Host: POSIX)\n", exName);
                system(RLTextFormat("make -C %s %s/%s PLATFORM=PLATFORM_DRM -B > %s/%s/logs/%s.build.log 2>&1",
                    exBasePath, exCategory, exName, exBasePath, exCategory, exName));
#else
                LOG("INFO: [%s] Building example for PLATFORM_DESKTOP (Host: POSIX)\n", exName);
                system(RLTextFormat("make -C %s %s/%s PLATFORM=PLATFORM_DESKTOP -B", exBasePath, exCategory, exName));
#endif
        ...
        case OP_UPDATE:
        {
            ...
            */
            ...
            RLFilePathList clist = RLLoadDirectoryFilesEx(exBasePath, ".c", true);

            // Load examples collection list file (raylib/examples/examples_list.txt)
            char *exList = RLLoadFileText(exCollectionFilePath);
            ...
            for (unsigned int i = 0; i < clist.count; i++)
            {
                ...
                if (!RLTextIsEqual(RLGetFileNameWithoutExt(clist.paths[i]), "examples_template") &&
                    (RLTextFindIndex(exList, RLGetFileNameWithoutExt(clist.paths[i])) == -1))
                {
                    ...
                    if (catIndex > -1)
                    {
                        ...
                        if (nextCatIndex != -1) exListNextCatIndex = RLTextFindIndex(exList, exCategories[nextCatIndex]);
                        ...
                        int exListNewExLen = sprintf(exListUpdated + exListNextCatIndex,
                            RLTextFormat("%s;%s;%s;%s;%s;%i;%i;\"%s\";@%s\n",
            ...
            */
            ...
            if (listUpdated) RLSaveFileText(exCollectionFilePath, exListUpdated);

            RLUnloadFileText(exList);
            RL_FREE(exListUpdated);

            RLUnloadDirectoryFiles(clist);
            //---------------------------------------------------------------------------------------------------
            ...
            for (int i = 0; i < exCollectionCount; i++)
            {
                ...
                if (!RLFileExists(RLTextFormat("%s/%s/%s.c", exBasePath, exInfo->category, exInfo->name))) exInfo->status |= VALID_MISSING_C;

                // Validate: raylib/examples/<category>/<category>_example_name.png     -> File exists?
                if (!RLFileExists(RLTextFormat("%s/%s/%s.png", exBasePath, exInfo->category, exInfo->name))) exInfo->status |= VALID_MISSING_PNG;

                // Validate: example screenshot is not the template default one
                RLImage imScreenshot = RLLoadImage(RLTextFormat("%s/%s/%s.png", exBasePath, exInfo->category, exInfo->name));
                RLImage imTemplate = RLLoadImage(RLTextFormat("%s/examples_template.png", exBasePath));
                if (memcmp(imScreenshot.data, imTemplate.data, RLGetPixelDataSize(imScreenshot.width, imScreenshot.height, imScreenshot.format)) == 0)
                    exInfo->status |= VALID_INVALID_PNG;
                RLUnloadImage(imTemplate);
                RLUnloadImage(imScreenshot);

                // Validate: raylib/examples/Makefile                                   -> Example listed?
                if (RLFileTextFindIndex(RLTextFormat("%s/Makefile", exBasePath), exInfo->name) == -1) exInfo->status |= VALID_NOT_IN_MAKEFILE;

                // Validate: raylib/examples/Makefile.Web                               -> Example listed?
                if (RLFileTextFindIndex(RLTextFormat("%s/Makefile.Web", exBasePath), exInfo->name) == -1) exInfo->status |= VALID_NOT_IN_MAKEFILE_WEB;

                // Validate: raylib/examples/README.md                                  -> Example listed?
                if (RLFileTextFindIndex(RLTextFormat("%s/README.md", exBasePath), exInfo->name) == -1) exInfo->status |= VALID_NOT_IN_README;

                // Validate: raylib.com/common/examples.js                              -> Example listed?
                if (RLFileTextFindIndex(RLTextFormat("%s/../common/examples.js", exWebPath), exInfo->name + RLTextFindIndex(exInfo->name, "_") + 1) == -1) exInfo->status |= VALID_NOT_IN_JS;

                // Validate: raylib/projects/VS2022/examples/<category>_example_name.vcxproj -> File exists?
                if (!RLFileExists(RLTextFormat("%s/../projects/VS2022/examples/%s.vcxproj", exBasePath, exInfo->name))) exInfo->status |= VALID_MISSING_VCXPROJ;

                // Validate: raylib/projects/VS2022/raylib.sln                          -> Example listed?
                if (RLFileTextFindIndex(RLTextFormat("%s/../projects/VS2022/raylib.sln", exBasePath), exInfo->name) == -1) exInfo->status |= VALID_NOT_IN_VCXSOL;
                ...
                char **resPaths = LoadExampleResourcePaths(RLTextFormat("%s/%s/%s.c", exBasePath, exInfo->category, exInfo->name), &exInfo->resCount);
                if (exInfo->resCount > 0)
                {
                    for (int r = 0; r < exInfo->resCount; r++)
                    {
                        ...
                        if (RLTextFindIndex(resPaths[r], "glsl%i") > -1)
                        {
                            ...
                            {
                                char *resPathUpdated = RLTextReplace(resPaths[r], "glsl%i", RLTextFormat("glsl%i", glslVer[v]));
                                if (!RLFileExists(RLTextFormat("%s/%s/%s", exBasePath, exInfo->category, resPathUpdated)))
                                {
                        ...
                        {
                            if (!RLFileExists(RLTextFormat("%s/%s/%s", exBasePath, exInfo->category, resPaths[r])))
                            {
                                exInfo->status |= VALID_MISSING_RESOURCES;
                ...
                if (!RLTextIsEqual(exInfo->category, "others") &&
                    (!RLFileExists(RLTextFormat("%s/%s/%s.html", exWebPath, exInfo->category, exInfo->name)) ||
                     !RLFileExists(RLTextFormat("%s/%s/%s.wasm", exWebPath, exInfo->category, exInfo->name)) ||
                     !RLFileExists(RLTextFormat("%s/%s/%s.js", exWebPath, exInfo->category, exInfo->name)) ||
                     ((exInfo->resCount > 0) && !RLFileExists(RLTextFormat("%s/%s/%s.data", exWebPath, exInfo->category, exInfo->name)))))
                {
                     exInfo->status |= VALID_MISSING_WEB_OUTPUT;
                }

                // Validate: raylib.com/examples/<category>/<category>_example_name.html -> Metadata
                if (RLFileExists(RLTextFormat("%s/%s/%s.html", exWebPath, exInfo->category, exInfo->name)))
                {
                    char *exHtmlText = RLLoadFileText(RLTextFormat("%s/%s/%s.html", exWebPath, exInfo->category, exInfo->name));

                    if ((RLTextFindIndex(exHtmlText, "raylib web game") > -1) || // title
                        (RLTextFindIndex(exHtmlText, "New raylib web videogame, developed using raylib videogames library") > -1) || // description
                        (RLTextFindIndex(exHtmlText, "https://www.raylib.com/common/raylib_logo.png") > -1) || // image
                        (RLTextFindIndex(exHtmlText, "https://www.raylib.com/games.html") > -1) || // url
                        (RLTextFindIndex(exHtmlText, "https://github.com/raysan5/raylib';") > -1)) // source code button
                    {
                        exInfo->status |= VALID_MISSING_WEB_METADATA;
                    }

                    RLUnloadFileText(exHtmlText);
                }

                // NOTE: Additional validation elements
                // Validate: Example naming conventions: <category>/<category>_example_name, valid category
                if ((RLTextFindIndex(exInfo->name, exInfo->category) == -1) ||
                    (!RLTextIsEqual(exInfo->category, "core") && !RLTextIsEqual(exInfo->category, "shapes") &&
                     !RLTextIsEqual(exInfo->category, "textures") && !RLTextIsEqual(exInfo->category, "text") &&
                     !RLTextIsEqual(exInfo->category, "models") && !RLTextIsEqual(exInfo->category, "shaders") &&
                     !RLTextIsEqual(exInfo->category, "audio") && !RLTextIsEqual(exInfo->category, "others"))) exInfo->status |= VALID_INVALID_CATEGORY;

                // Validate: Example info (stars, author, github) missmatches with example header content
                rlExampleInfo *exInfoHeader = LoadExampleInfo(RLTextFormat("%s/%s/%s.c", exBasePath, exInfo->category, exInfo->name));
                ...
                for (int i = 0; i < exCollectionCount; i++)
                {
                    ...
                    else
                    {
                        ...
                        if (exInfo->status & VALID_MISSING_VCXPROJ)
                        {
                            ...
                            RLFileCopy(RLTextFormat("%s/../projects/VS2022/examples/core_basic_window.vcxproj", exBasePath),
                                RLTextFormat("%s/../projects/VS2022/examples/%s.vcxproj", exBasePath, exInfo->name));
                            RLFileTextReplace(RLTextFormat("%s/../projects/VS2022/examples/%s.vcxproj", exBasePath, exInfo->name),
                                "core_basic_window", exInfo->name);
                            RLFileTextReplace(RLTextFormat("%s/../projects/VS2022/examples/%s.vcxproj", exBasePath, exInfo->name),
                                "..\\..\\examples\\core", RLTextFormat("..\\..\\examples\\%s", exInfo->category));
                        ...
                        if (exInfo->status & VALID_NOT_IN_VCXSOL)
                        {
                            ...
                            AddVSProjectToSolution(exVSProjectSolutionFile,
                                RLTextFormat("%s/../projects/VS2022/examples/%s.vcxproj", exBasePath, exInfo->name), exInfo->category);
                            ...
                            ((exInfo->status & VALID_MISSING_WEB_OUTPUT) || (exInfo->status & VALID_MISSING_WEB_METADATA)))
                        {
                            ...
                            LOG("INFO: [%s] Building example for PLATFORM_WEB (Host: Win32)\n", exInfo->name);
                            _putenv("PATH=%PATH%;C:\\raylib\\w64devkit\\bin");
                            system(RLTextFormat("mingw32-make -C %s -f Makefile.Web %s/%s PLATFORM=PLATFORM_WEB -B", exBasePath, exInfo->category, exInfo->name));
                        #else
                            LOG("INFO: [%s] Building example for PLATFORM_WEB (Host: POSIX)\n", exInfo->name);
                            system(RLTextFormat("make -C %s -f Makefile.Web %s/%s PLATFORM=PLATFORM_WEB -B", exBasePath, exInfo->category, exInfo->name));
                        #endif

                            // Update generated .html metadata
                            LOG("INFO: [%s.html] Updating HTML Metadata...\n", exInfo->name);
                            UpdateWebMetadata(RLTextFormat("%s/%s/%s.html", exBasePath, exInfo->category, exInfo->name),
                                RLTextFormat("%s/%s/%s.c", exBasePath, exInfo->category, exInfo->name));

                            // Copy results to web side
                            LOG("INFO: [%s] Copy example build to raylib.com\n", exInfo->name);
                            RLFileCopy(RLTextFormat("%s/%s/%s.html", exBasePath, exInfo->category, exInfo->name),
                                RLTextFormat("%s/%s/%s.html", exWebPath, exInfo->category, exInfo->name));
                            RLFileCopy(RLTextFormat("%s/%s/%s.data", exBasePath, exInfo->category, exInfo->name),
                                RLTextFormat("%s/%s/%s.data", exWebPath, exInfo->category, exInfo->name));
                            RLFileCopy(RLTextFormat("%s/%s/%s.wasm", exBasePath, exInfo->category, exInfo->name),
                                RLTextFormat("%s/%s/%s.wasm", exWebPath, exInfo->category, exInfo->name));
                            RLFileCopy(RLTextFormat("%s/%s/%s.js", exBasePath, exInfo->category, exInfo->name),
                                RLTextFormat("%s/%s/%s.js", exWebPath, exInfo->category, exInfo->name));
                        ...
                        if (exInfo->status & VALID_INCONSISTENT_INFO)
                        {
                            ...
                            UpdateSourceMetadata(RLTextFormat("%s/%s/%s.c", exBasePath, exInfo->category, exInfo->name), exInfo);
            ...
            */
            ...
            RLSaveFileText(RLTextFormat("%s/../tools/rexm/reports/%s", exBasePath, "examples_validation.md"), report);
            RL_FREE(report);
            //-----------------------------------------------------------------------------------------------------
            ...
            RLSaveFileText(RLTextFormat("%s/../tools/rexm/reports/%s", exBasePath, "examples_issues.md"), reportIssues);
        ...
        case OP_TEST:
        {
            ...
            for (int i = 0; i < exBuildListCount; i++)
            {
                ...
                RLMakeDirectory(RLTextFormat("%s/%s/logs", exBasePath, exCategory));
                ...
                RLFileCopy(RLTextFormat("%s/%s/%s.c", exBasePath, exCategory, exName),
                    RLTextFormat("%s/%s/%s.original.c", exBasePath, exCategory, exName));
                char *srcText = RLLoadFileText(RLTextFormat("%s/%s/%s.c", exBasePath, exCategory, exName));
...
//#define BUILD_TESTING_WEB
                ...
                static const char *returnReplaceText =
                    ...
                    "    return 0";
                char *returnReplaceTextUpdated = RLTextReplace(returnReplaceText, "outputLogFileName", RLTextFormat("%s.log", exName));

                char *srcTextUpdated[4] = { 0 };
                srcTextUpdated[0] = RLTextReplace(srcText, "int main(void)\n{", mainReplaceText);
                srcTextUpdated[1] = RLTextReplace(srcTextUpdated[0], "WindowShouldClose()", "WindowShouldClose() && (testFramesCount < requestedTestFrames)");
                srcTextUpdated[2] = RLTextReplace(srcTextUpdated[1], "EndDrawing();", "EndDrawing(); testFramesCount++;");
                srcTextUpdated[3] = RLTextReplace(srcTextUpdated[2], "    return 0", returnReplaceTextUpdated);
                RLMemFree(returnReplaceTextUpdated);
                RLUnloadFileText(srcText);

                RLSaveFileText(RLTextFormat("%s/%s/%s.c", exBasePath, exCategory, exName), srcTextUpdated[3]);
                for (int i = 0; i < 4; i++) { RLMemFree(srcTextUpdated[i]); srcTextUpdated[i] = NULL; }
    ...
    #if defined(_WIN32)
                LOG("INFO: [%s] Building example for PLATFORM_WEB (Host: Win32)\n", exName);
                system(RLTextFormat("mingw32-make -C %s -f Makefile.Web %s/%s PLATFORM=PLATFORM_WEB -B > %s/%s/logs/%s.build.log 2>&1",
                    exBasePath, exCategory, exName, exBasePath, exCategory, exName));
    #else
                LOG("INFO: [%s] Building example for PLATFORM_WEB (Host: POSIX)\n", exName);
                system(RLTextFormat("make -C %s -f Makefile.Web %s/%s PLATFORM=PLATFORM_WEB -B", exBasePath, exCategory, exName));
                ...
                RLFileCopy(RLTextFormat("%s/%s/%s.original.c", exBasePath, exCategory, exName),
                    RLTextFormat("%s/%s/%s.c", exBasePath, exCategory, exName));
                RLFileRemove(RLTextFormat("%s/%s/%s.original.c", exBasePath, exCategory, exName));

                // STEP 3: Run example on browser
                if (RLFileExists(RLTextFormat("%s/%s/%s.html", exBasePath, exCategory, exName)) &&
                    RLFileExists(RLTextFormat("%s/%s/%s.wasm", exBasePath, exCategory, exName)) &&
                    RLFileExists(RLTextFormat("%s/%s/%s.js", exBasePath, exCategory, exName)))
                {
                    ...
                    RLChangeDirectory(RLTextFormat("%s", exBasePath));
                    if (i == 0) system("start python -m http.server 8080"); // Init localhost just once
                    system(RLTextFormat("start explorer \"http:\\localhost:8080/%s/%s.html", exCategory, exName));
                }
                ...
                RLSaveFileText(RLTextFormat("%s/%s/%s.c", exBasePath, exCategory, exName), srcTextUpdated[2]);
                for (int i = 0; i < 3; i++) { RLMemFree(srcTextUpdated[i]); srcTextUpdated[i] = NULL; }
    ...
    #if defined(_WIN32)
                LOG("INFO: [%s] Building example for PLATFORM_DESKTOP (Host: Win32)\n", exName);
                system(RLTextFormat("mingw32-make -C %s %s/%s PLATFORM=PLATFORM_DESKTOP -B > %s/%s/logs/%s.build.log 2>&1",
                    exBasePath, exCategory, exName, exBasePath, exCategory, exName));
    #elif defined(PLATFORM_DRM)
                LOG("INFO: [%s] Building example for PLATFORM_DRM (Host: POSIX)\n", exName);
                system(RLTextFormat("make -C %s %s/%s PLATFORM=PLATFORM_DRM -B > %s/%s/logs/%s.build.log 2>&1",
    ...
    #else
                LOG("INFO: [%s] Building example for PLATFORM_DESKTOP (Host: POSIX)\n", exName);
                system(RLTextFormat("make -C %s %s/%s PLATFORM=PLATFORM_DESKTOP -B > %s/%s/logs/%s.build.log 2>&1",
                    exBasePath, exCategory, exName, exBasePath, exCategory, exName));
    #endif
                // Restore original source code before continue
                RLFileCopy(RLTextFormat("%s/%s/%s.original.c", exBasePath, exCategory, exName),
                ...
                RLChangeDirectory(RLTextFormat("%s/%s", exBasePath, exCategory));

    #if defined(_WIN32)
                system(RLTextFormat("%s --frames 2 > logs/%s.log", exName, exName));
    #else
                system(RLTextFormat("./%s --frames 2 > logs/%s.log", exName, exName));
    #endif
#endif
        ...
        case OP_TESTLOG:
        {
            ...
            for (int i = 0; i < exBuildListCount; i++)
            {
                ...
                char *exTestBuildLog = RLLoadFileText(RLTextFormat("%s/%s/logs/%s.build.log", exBasePath, exCategory, exName));
                ...
                char **exTestBuildLogLines = RLLoadTextLines(exTestBuildLog, &exTestBuildLogLinesCount);

                for (int k = 0; k < exTestBuildLogLinesCount; k++)
                {
                    // Checking compilation warnings generated
                    if (RLTextFindIndex(exTestBuildLogLines[k], "warning:") >= 0) testing[i].buildwarns++;
                }

                RLUnloadTextLines(exTestBuildLogLines, exTestBuildLogLinesCount);
                RLUnloadFileText(exTestBuildLog);

#if defined(BUILD_TESTING_WEB)
                // TODO: REVIEW: Hardcoded path where web logs are copied after automatic download
                char *exTestLog = RLLoadFileText(RLTextFormat("D:/testing_logs_web/%s.log", exName));
#else
                char *exTestLog = RLLoadFileText(RLTextFormat("%s/%s/logs/%s.log", exBasePath, exCategory, exName));
#endif
                if (exTestLog == NULL)
                {
                ...
                */
                ...
                if (RLTextFindIndex(exTestLog, "INFO: DISPLAY: Device initialized successfully") == -1) testing[i].status |= TESTING_FAIL_INIT;
                if (RLTextFindIndex(exTestLog, "INFO: Window closed successfully") == -1) testing[i].status |= TESTING_FAIL_CLOSE;
                if (RLTextFindIndex(exTestLog, "WARNING: FILEIO:") >= 0) testing[i].status |= TESTING_FAIL_ASSETS;
                if (RLTextFindIndex(exTestLog, "INFO: RLGL: Default OpenGL state initialized successfully") == -1) testing[i].status |= TESTING_FAIL_RLGL;
                if (RLTextFindIndex(exTestLog, "INFO: PLATFORM:") == -1) testing[i].status |= TESTING_FAIL_PLATFORM;
                if (RLTextFindIndex(exTestLog, "INFO: FONT: Default font loaded successfully") == -1) testing[i].status |= TESTING_FAIL_FONT;
                if (RLTextFindIndex(exTestLog, "INFO: TIMER: Target time per frame:") == -1) testing[i].status |= TESTING_FAIL_TIMER;

                // Load build log text lines
                int exTestLogLinesCount = 0;
                char **exTestLogLines = RLLoadTextLines(exTestLog, &exTestLogLinesCount);
                for (int k = 0; k < exTestLogLinesCount; k++)
                {
#if defined(BUILD_TESTING_WEB)
                    if (RLTextFindIndex(exTestLogLines[k], "WARNING: GL: NPOT") >= 0) continue; // Ignore web-specific warning
#endif
#if defined(PLATFORM_DRM)
                    if (RLTextFindIndex(exTestLogLines[k], "WARNING: DISPLAY: No graphic") >= 0) continue; // Ignore specific warning
                    if (RLTextFindIndex(exTestLogLines[k], "WARNING: GetCurrentMonitor()") >= 0) continue; // Ignore specific warning
                    if (RLTextFindIndex(exTestLogLines[k], "WARNING: SetWindowPosition()") >= 0) continue; // Ignore specific warning
#endif
                    if (RLTextFindIndex(exTestLogLines[k], "WARNING") >= 0) testing[i].warnings++;
                }
                RLUnloadTextLines(exTestLogLines, exTestLogLinesCount);
                RLUnloadFileText(exTestLog);
            }
            //---------------------------------------------------------------------------------------------
            ...
            */
            ...
            repIndex += sprintf(report + repIndex, RLTextFormat("## Tested Platform: %s\n\n", osName));
            ...
            */
            ...
            RLSaveFileText(RLTextFormat("%s/../tools/rexm/reports/examples_testing_%s.md", exBasePath, RLTextToLower(osName)), report);
...
static int UpdateRequiredFiles(void)
{
    ...
    for (int i = 0; i < exListCount; i++)
    {
        ...
        UpdateSourceMetadata(RLTextFormat("%s/%s/%s.c", exBasePath, info->category, info->name), info);
    }
    UnloadExampleData(exList);
    //------------------------------------------------------------------------------------------------
    ...
    char *mkText = RLLoadFileText(RLTextFormat("%s/Makefile", exBasePath));
    char *mkTextUpdated = (char *)RL_CALLOC(REXM_MAX_BUFFER_SIZE, 1); // Updated Makefile copy, 2MB

    int mkListStartIndex = RLTextFindIndex(mkText, "#EXAMPLES_LIST_START");
    int mkListEndIndex = RLTextFindIndex(mkText, "#EXAMPLES_LIST_END");
    ...
    for (int i = 0; i < REXM_MAX_EXAMPLE_CATEGORIES; i++)
    {
        mkIndex += sprintf(mkTextUpdated + mkListStartIndex + mkIndex, RLTextFormat("%s = \\\n", RLTextToUpper(exCategories[i])));

        int exCollectionCount = 0;
        rlExampleInfo *exCollection = LoadExampleData(exCategories[i], true, &exCollectionCount);

        for (int x = 0; x < exCollectionCount - 1; x++) mkIndex += sprintf(mkTextUpdated + mkListStartIndex + mkIndex, RLTextFormat("    %s/%s \\\n", exCollection[x].category, exCollection[x].name));
        mkIndex += sprintf(mkTextUpdated + mkListStartIndex + mkIndex, RLTextFormat("    %s/%s\n\n", exCollection[exCollectionCount - 1].category, exCollection[exCollectionCount - 1].name));
    ...
    // Save updated file
    RLSaveFileText(RLTextFormat("%s/Makefile", exBasePath), mkTextUpdated);
    RLUnloadFileText(mkText);
    ...
    char *mkwText = RLLoadFileText(RLTextFormat("%s/Makefile.Web", exBasePath));
    char *mkwTextUpdated = (char *)RL_CALLOC(REXM_MAX_BUFFER_SIZE, 1); // Updated Makefile copy, 2MB

    int mkwListStartIndex = RLTextFindIndex(mkwText, "#EXAMPLES_LIST_START");
    int mkwListEndIndex = RLTextFindIndex(mkwText, "#EXAMPLES_LIST_END");
    ...
    {
        mkwIndex += sprintf(mkwTextUpdated + mkwListStartIndex + mkwIndex, RLTextFormat("%s = \\\n", RLTextToUpper(exCategories[i])));

        int exCollectionCount = 0;
        rlExampleInfo *exCollection = LoadExampleData(exCategories[i], true, &exCollectionCount);

        for (int x = 0; x < exCollectionCount - 1; x++) mkwIndex += sprintf(mkwTextUpdated + mkwListStartIndex + mkwIndex, RLTextFormat("    %s/%s \\\n", exCollection[x].category, exCollection[x].name));
        mkwIndex += sprintf(mkwTextUpdated + mkwListStartIndex + mkwIndex, RLTextFormat("    %s/%s\n\n", exCollection[exCollectionCount - 1].category, exCollection[exCollectionCount - 1].name));
    ...
    for (int i = 0; i < REXM_MAX_EXAMPLE_CATEGORIES - 1; i++)
    {
        mkwIndex += sprintf(mkwTextUpdated + mkwListStartIndex + mkwIndex, RLTextFormat("# Compile %s examples\n", RLTextToUpper(exCategories[i])));
        ...
        for (int x = 0; x < exCollectionCount; x++)
        {
            ...
            int resPathCount = 0;
            char **resPaths = LoadExampleResourcePaths(RLTextFormat("%s/%s/%s.c", exBasePath, exCollection[x].category, exCollection[x].name), &resPathCount);
            ...
            if (resPathCount > 0)
            {
                ...
                */
                ...
                for (int r = 0; r < resPathCount; r++)
                {
                    ...
                    if (RLTextFindIndex(resPaths[r], "glsl%i") > -1)
                    {
                        char *resPathUpdated = RLTextReplace(resPaths[r], "glsl%i", "glsl100");
    ...
    // Save updated file
    RLSaveFileText(RLTextFormat("%s/Makefile.Web", exBasePath), mkwTextUpdated);
    RLUnloadFileText(mkwText);
    ...
    char *mdText = RLLoadFileText(RLTextFormat("%s/README.md", exBasePath));
    char *mdTextUpdated = (char *)RL_CALLOC(REXM_MAX_BUFFER_SIZE, 1); // Updated examples.js copy, 2MB

    int mdListStartIndex = RLTextFindIndex(mdText, "## EXAMPLES COLLECTION");
    ...
    mdIndex += sprintf(mdTextUpdated + mdListStartIndex + mdIndex, RLTextFormat("## EXAMPLES COLLECTION [TOTAL: %i]\n", exCollectionFullCount));
    ...
    for (int i = 0; i < REXM_MAX_EXAMPLE_CATEGORIES; i++)
    {
        ...
        if (i == 0)         // "core"
        {
            mdIndex += sprintf(mdTextUpdated + mdListStartIndex + mdIndex, RLTextFormat("\n### category: core [%i]\n\n", exCollectionCount));
        ...
        for (int x = 0; x < exCollectionCount; x++)
        {
            ...
            mdIndex += sprintf(mdTextUpdated + mdListStartIndex + mdIndex,
                RLTextFormat("| [%s](%s/%s.c) | <img src=\"%s/%s.png\" alt=\"%s\" width=\"80\"> | %s | %s | %s | [%s](https://github.com/%s) |\n",
    ...
    // Save updated file
    RLSaveFileText(RLTextFormat("%s/README.md", exBasePath), mdTextUpdated);
    RLUnloadFileText(mdText);
    ...
    char *jsText = RLLoadFileText(RLTextFormat("%s/../common/examples.js", exWebPath));
    if (!jsText)
    {
    ...
    {
        int jsListStartIndex = RLTextFindIndex(jsText, "//EXAMPLE_DATA_LIST_START");
        int jsListEndIndex = RLTextFindIndex(jsText, "//EXAMPLE_DATA_LIST_END");
        if ((jsListStartIndex < 0) || (jsListEndIndex < 0))
        {
            LOG("WARNING: examples.js markers not found, skipping update\n");
            RLUnloadFileText(jsText);
        }
        else
        {
            ...
            // Save updated file
            RLSaveFileText(RLTextFormat("%s/../common/examples.js", exWebPath), jsTextUpdated);
            RLUnloadFileText(jsText);
...
static rlExampleInfo *LoadExampleData(const char *filter, bool sort, int *exCount)
{
    ...
    *exCount = 0;
    ...
    char *text = RLLoadFileText(exCollectionFilePath);
...
static rlExampleInfo *LoadExampleInfo(const char *exFileName)
{
    ...
    if (RLFileExists(exFileName) && RLIsFileExtension(exFileName, ".c"))
    {
        // Example found in collection
        exInfo = (rlExampleInfo *)RL_CALLOC(1, sizeof(rlExampleInfo));

        strncpy(exInfo->name, RLGetFileNameWithoutExt(exFileName), 128 - 1);
        strncpy(exInfo->category, exInfo->name, RLTextFindIndex(exInfo->name, "_"));

        char *exText = RLLoadFileText(exFileName);

        // Get example difficulty stars
        // NOTE: Counting the unicode char occurrences: ★
        int starsIndex = RLTextFindIndex(exText, "★");
        if (starsIndex > 0)
        {
        ...
        int verCreateIndex = RLTextFindIndex(exText, "created with raylib "); // Version = index + 20
        int verCreatedLen = 0;
        ...
        int verUpdateIndex = RLTextFindIndex(exText, "updated with raylib "); // Version = index + 20
        int verUpdateLen = 0;
        ...
        int copyrightIndex = RLTextFindIndex(exText, "Copyright (c) ");
        int yearStartIndex = copyrightIndex + 14;
        char yearText[5] = { 0 };
        strncpy(yearText, exText + yearStartIndex, 4);
        exInfo->yearCreated = RLTextToInteger(yearText);
        ...
        exInfo->yearReviewed = RLTextToInteger(yearText);
        int yearEndIndex = RLTextFindIndex(exText + yearStartIndex, " ");
        int authorStartIndex = yearStartIndex + yearEndIndex + 1;
        int authorEndIndex = RLTextFindIndex(exText + authorStartIndex, " (@");
        if (authorEndIndex != -1) // Github user also available
        {
            ...
            int userEndIndex = RLTextFindIndex(exText + userStartIndex, ")");
        ...
        {
            authorEndIndex = RLTextFindIndex(exText + authorStartIndex, "\n");
            authorEndIndex += authorStartIndex;
            strncpy(exInfo->author, exText + authorStartIndex, authorEndIndex - authorStartIndex);
        }
...
static int ParseExampleInfoLine(const char *line, rlExampleInfo *entry)
{
    ...
    int tokenCount = 0;
    char **tokens = RLTextSplit(line, ';', &tokenCount);
    ...
    entry->yearCreated = RLTextToInteger(tokens[5]);
    entry->yearReviewed = RLTextToInteger(tokens[6]);
...
static char **LoadExampleResourcePaths(const char *filePath, int *resPathCount)
{
    ...
    int resCounter = 0;
    char *code = RLLoadFileText(filePath);
    ...
    if (code != NULL)
    {
        ...
        while ((ptr = strchr(ptr, '"')) != NULL)
        {
            ...
            int functionIndex01 = RLTextFindIndex(ptr - 40, "ExportImage");       // Check ExportImage()
            int functionIndex02 = RLTextFindIndex(ptr - 10, "TraceLog");          // Check TraceLog()
            int functionIndex03 = RLTextFindIndex(ptr - 40, "TakeScreenshot");    // Check TakeScreenshot()
            int functionIndex04 = RLTextFindIndex(ptr - 40, "SaveFileData");      // Check SaveFileData()
            int functionIndex05 = RLTextFindIndex(ptr - 40, "SaveFileText");      // Check SaveFileText()
                ...
                !((functionIndex05 != -1) && (functionIndex05 < 40)))    // Not found SaveFileText() before ""
            {
                ...
                if ((len > 0) && (len < REXM_MAX_RESOURCE_PATH_LEN))
                {
                    ...
                    for (int i = 0; i < extCount; i++)
                    {
                        ...
                        if (RLIsFileExtension(buffer, exts[i]))
                        {
                            // Avoid duplicates
                            bool found = false;
...
static int AddVSProjectToSolution(const char *slnFile, const char *projFile, const char *category)
{
    ...
    RLFileTextReplace(projFile, "0981CA98-E4A5-4DF1-987F-A41D09131EFC", uuid);

    char *slnText = RLLoadFileText(slnFile);
    ...
    int prjStartIndex = RLTextFindIndex(slnText, "Global");
    ...
    int offsetIndex = sprintf(slnTextUpdated + prjStartIndex,
        RLTextFormat("Project(\"{8BC9CEB8-8B4A-11D0-8D11-00A0C91BC942}\") = \"%s\", \"examples\\%s\", \"{%s}\"\n",
            RLGetFileNameWithoutExt(projFile), RLGetFileName(projFile), uuid));
    ...
    int projConfStartIndex = RLTextFindIndex(slnText, "GlobalSection(ProjectConfigurationPlatforms) = postSolution");
    ...
    int projConfEndIndex = RLTextFindIndex(slnText + projConfStartIndex, "EndGlobalSection");
    ...
    offsetIndex += sprintf(slnTextUpdated + offsetIndex, RLTextFormat("\t{%s}.Debug.DLL|ARM64.ActiveCfg = Debug.DLL|ARM64\n", uuid));
    offsetIndex += sprintf(slnTextUpdated + offsetIndex, RLTextFormat("\t\t{%s}.Debug.DLL|ARM64.Build.0 = Debug.DLL|ARM64\n", uuid));
    offsetIndex += sprintf(slnTextUpdated + offsetIndex, RLTextFormat("\t\t{%s}.Debug.DLL|x64.ActiveCfg = Debug.DLL|x64\n", uuid));
    offsetIndex += sprintf(slnTextUpdated + offsetIndex, RLTextFormat("\t\t{%s}.Debug.DLL|x64.Build.0 = Debug.DLL|x64\n", uuid));
    offsetIndex += sprintf(slnTextUpdated + offsetIndex, RLTextFormat("\t\t{%s}.Debug.DLL|x86.ActiveCfg = Debug.DLL|Win32\n", uuid));
    offsetIndex += sprintf(slnTextUpdated + offsetIndex, RLTextFormat("\t\t{%s}.Debug.DLL|x86.Build.0 = Debug.DLL|Win32\n", uuid));
    offsetIndex += sprintf(slnTextUpdated + offsetIndex, RLTextFormat("\t\t{%s}.Debug|ARM64.ActiveCfg = Debug|ARM64\n", uuid));
    offsetIndex += sprintf(slnTextUpdated + offsetIndex, RLTextFormat("\t\t{%s}.Debug|ARM64.Build.0 = Debug|ARM64\n", uuid));
    offsetIndex += sprintf(slnTextUpdated + offsetIndex, RLTextFormat("\t\t{%s}.Debug|x64.ActiveCfg = Debug|x64\n", uuid));
    offsetIndex += sprintf(slnTextUpdated + offsetIndex, RLTextFormat("\t\t{%s}.Debug|x64.Build.0 = Debug|x64\n", uuid));
    offsetIndex += sprintf(slnTextUpdated + offsetIndex, RLTextFormat("\t\t{%s}.Debug|x86.ActiveCfg = Debug|Win32\n", uuid));
    offsetIndex += sprintf(slnTextUpdated + offsetIndex, RLTextFormat("\t\t{%s}.Debug|x86.Build.0 = Debug|Win32\n", uuid));
    offsetIndex += sprintf(slnTextUpdated + offsetIndex, RLTextFormat("\t\t{%s}.Release.DLL|ARM64.ActiveCfg = Release.DLL|ARM64\n", uuid));
    offsetIndex += sprintf(slnTextUpdated + offsetIndex, RLTextFormat("\t\t{%s}.Release.DLL|ARM64.Build.0 = Release.DLL|ARM64\n", uuid));
    offsetIndex += sprintf(slnTextUpdated + offsetIndex, RLTextFormat("\t\t{%s}.Release.DLL|x64.ActiveCfg = Release.DLL|x64\n", uuid));
    offsetIndex += sprintf(slnTextUpdated + offsetIndex, RLTextFormat("\t\t{%s}.Release.DLL|x64.Build.0 = Release.DLL|x64\n", uuid));
    offsetIndex += sprintf(slnTextUpdated + offsetIndex, RLTextFormat("\t\t{%s}.Release.DLL|x86.ActiveCfg = Release.DLL|Win32\n", uuid));
    offsetIndex += sprintf(slnTextUpdated + offsetIndex, RLTextFormat("\t\t{%s}.Release.DLL|x86.Build.0 = Release.DLL|Win32\n", uuid));
    offsetIndex += sprintf(slnTextUpdated + offsetIndex, RLTextFormat("\t\t{%s}.Release|ARM64.ActiveCfg = Release|ARM64\n", uuid));
    offsetIndex += sprintf(slnTextUpdated + offsetIndex, RLTextFormat("\t\t{%s}.Release|ARM64.Build.0 = Release|ARM64\n", uuid));
    offsetIndex += sprintf(slnTextUpdated + offsetIndex, RLTextFormat("\t\t{%s}.Release|x64.ActiveCfg = Release|x64\n", uuid));
    offsetIndex += sprintf(slnTextUpdated + offsetIndex, RLTextFormat("\t\t{%s}.Release|x64.Build.0 = Release|x64\n", uuid));
    offsetIndex += sprintf(slnTextUpdated + offsetIndex, RLTextFormat("\t\t{%s}.Release|x86.ActiveCfg = Release|Win32\n", uuid));
    offsetIndex += sprintf(slnTextUpdated + offsetIndex, RLTextFormat("\t\t{%s}.Release|x86.Build.0 = Release|Win32\n", uuid));
    ...
    int projFolderStartIndex = RLTextFindIndex(slnText, "GlobalSection(NestedProjects) = preSolution");
    int projFolderEndIndex = RLTextFindIndex(slnText + projFolderStartIndex, "\tEndGlobalSection");
    ...
    RLSaveFileText(slnFile, slnTextUpdated);
    RLUnloadFileText(slnText);
...
static int RemoveVSProjectFromSolution(const char *slnFile, const char *exName)
{
    ...
    int lineCount = 0;
    char **lines = RLLoadTextLines(slnText, &lineCount); // WARNING: Max 512 lines, we need +4000!
    ...
    {
        index = RLTextFindIndex(lines[i], exName);
        if (index > 0)
        {
        ...
        {
            if (RLTextFindIndex(lines[i], uuid) == -1)
...
static const char *GenerateUUIDv4(void)
{
    ...
    for (int i = 0; i < 16; i++) bytes[i] = (unsigned char)RLGetRandomValue(0, 255);
...
static void UpdateSourceMetadata(const char *exSrcPath, const rlExampleInfo *info)
{
    if (RLFileExists(exSrcPath) && RLIsFileExtension(exSrcPath, ".c") && (!RLTextIsEqual(info->category, "others")))
    {
        ...
        char *exText = RLLoadFileText(exSourcePath);
        ...
        int exNameIndex = RLTextFindIndex(info->name, "_");
        ...
        exTextUpdated[0] = RLTextReplaceBetween(exTextUpdatedPtr, "*   raylib [", "\n",
            RLTextFormat("%s] example - %s", info->category, exNameFormated));
        ...
        exTextUpdated[1] = RLTextReplaceBetween(exTextUpdatedPtr, "*   Example complexity rating: [", "/4\n",
            RLTextFormat("%s] %i", starsText, info->stars));
        ...
        exTextUpdated[2] = RLTextReplaceBetween(exTextUpdatedPtr, "*   Example originally created with raylib ", "\n",
            RLTextFormat("%s, last time updated with raylib %s", info->verCreated, info->verUpdated));
        ...
        {
            exTextUpdated[3] = RLTextReplaceBetween(exTextUpdatedPtr, "Copyright (c) ", ")",
                RLTextFormat("%i %s (@%s", info->yearCreated, info->author, info->authorGitHub));
            if (exTextUpdated[3] != NULL) exTextUpdatedPtr = exTextUpdated[3];
        }
        else
        {
            exTextUpdated[3] = RLTextReplaceBetween(exTextUpdatedPtr, "Copyright (c) ", ")",
                RLTextFormat("%i-%i %s (@%s", info->yearCreated, info->yearReviewed, info->author, info->authorGitHub));
            if (exTextUpdated[3] != NULL) exTextUpdatedPtr = exTextUpdated[3];
        }

        // Update window title
        // String: "InitWindow(screenWidth, screenHeight, "raylib [shaders] example - texture drawing");"
        exTextUpdated[4] = RLTextReplaceBetween(exTextUpdated[3], "InitWindow(screenWidth, screenHeight, \"", "\");",
            RLTextFormat("raylib [%s] example - %s", info->category, exNameFormated));
        ...
        exTextUpdated[5] = RLTextReplaceBetween(exTextUpdatedPtr, "*   Example contributed by ", ")",
            RLTextFormat("%s (@%s", info->author, info->authorGitHub));
        if (exTextUpdated[5] != NULL) exTextUpdatedPtr = exTextUpdated[5];

        if (exTextUpdatedPtr != NULL) RLSaveFileText(exSourcePath, exTextUpdatedPtr);
...
static void UpdateWebMetadata(const char *exHtmlPath, const char *exFilePath)
{
    if (RLFileExists(exHtmlPath) && RLIsFileExtension(exHtmlPath, ".html"))
    {
        ...
        char *exHtmlText = RLLoadFileText(exHtmlPathCopy);
        ...
        strncpy(exName, RLGetFileNameWithoutExt(exHtmlPathCopy), 64 - 1);
        ...
        char *exText = RLLoadFileText(exFilePath);
        int lineCount = 0;
        char **lines = RLLoadTextLines(exText, &lineCount);
        ...
        exHtmlTextUpdated[0] = RLTextReplace(exHtmlText, "raylib web game", exTitle);
        exHtmlTextUpdated[1] = RLTextReplace(exHtmlTextUpdated[0], "New raylib web videogame, developed using raylib videogames library", exDescription);
        exHtmlTextUpdated[2] = RLTextReplace(exHtmlTextUpdated[1], "https://www.raylib.com/common/raylib_logo.png",
            RLTextFormat("https://raw.githubusercontent.com/raysan5/raylib/master/examples/%s/%s.png", exCategory, exName));
        exHtmlTextUpdated[3] = RLTextReplace(exHtmlTextUpdated[2], "https://www.raylib.com/games.html",
            RLTextFormat("https://www.raylib.com/examples/%s/%s.html", exCategory, exName));
        exHtmlTextUpdated[4] = RLTextReplace(exHtmlTextUpdated[3], "raylib - example", RLTextFormat("raylib - %s", exName)); // og:site_name
        exHtmlTextUpdated[5] = RLTextReplace(exHtmlTextUpdated[4], "https://github.com/raysan5/raylib",
            RLTextFormat("https://github.com/raysan5/raylib/blob/master/examples/%s/%s.c", exCategory, exName));

        RLSaveFileText(exHtmlPathCopy, exHtmlTextUpdated[5]);
        ...
        for (int i = 0; i < 6; i++) { RLMemFree(exHtmlTextUpdated[i]); exHtmlTextUpdated[i] = NULL; }
...
static int GetTextListIndex(const char *text, const char **list, int listCount)
{
    ...
    {
        if (RLTextIsEqual(text, list[i])) { result = i; break; }
    }
