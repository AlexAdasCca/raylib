...
typedef struct {
    ...
    RLVector2 touchPosition[MAX_TOUCH_POINTS]; // Track touch positions for each slot
...
static const short linuxToRaylibMap[KEYMAP_SIZE] = {
    ...
    [BTN_DPAD_UP] = RL_E_GAMEPAD_BUTTON_LEFT_FACE_UP,
    [BTN_DPAD_RIGHT] = RL_E_GAMEPAD_BUTTON_LEFT_FACE_RIGHT,
    [BTN_DPAD_DOWN] = RL_E_GAMEPAD_BUTTON_LEFT_FACE_DOWN,
    [BTN_DPAD_LEFT] = RL_E_GAMEPAD_BUTTON_LEFT_FACE_LEFT,
    [BTN_Y] = RL_E_GAMEPAD_BUTTON_RIGHT_FACE_UP,
    [BTN_B] = RL_E_GAMEPAD_BUTTON_RIGHT_FACE_RIGHT,
    [BTN_A] = RL_E_GAMEPAD_BUTTON_RIGHT_FACE_DOWN,
    [BTN_X] = RL_E_GAMEPAD_BUTTON_RIGHT_FACE_LEFT,
    [BTN_TL] = RL_E_GAMEPAD_BUTTON_LEFT_TRIGGER_1,
    [BTN_TL2] = RL_E_GAMEPAD_BUTTON_LEFT_TRIGGER_2,
    [BTN_TR] = RL_E_GAMEPAD_BUTTON_RIGHT_TRIGGER_1,
    [BTN_TR2] = RL_E_GAMEPAD_BUTTON_RIGHT_TRIGGER_2,
    [BTN_SELECT] = RL_E_GAMEPAD_BUTTON_MIDDLE_LEFT,
    [BTN_MODE] = RL_E_GAMEPAD_BUTTON_MIDDLE,
    [BTN_START] = RL_E_GAMEPAD_BUTTON_MIDDLE_RIGHT,
    [BTN_THUMBL] = RL_E_GAMEPAD_BUTTON_LEFT_THUMB,
    [BTN_THUMBR] = RL_E_GAMEPAD_BUTTON_RIGHT_THUMB,
};
...
int RLGetMonitorWidth(int monitor)
{
    ...
    if (monitor != 0)
    {
        TRACELOG(RL_E_LOG_WARNING, "GetMonitorWidth() implemented for first monitor only");
    }
...
int RLGetMonitorHeight(int monitor)
{
    ...
    if (monitor != 0)
    {
        TRACELOG(RL_E_LOG_WARNING, "GetMonitorHeight() implemented for first monitor only");
    }
    else if ((platform.connector) && (platform.modeIndex >= 0))
    {
...
int RLGetMonitorPhysicalWidth(int monitor)
{
    ...
    if (monitor != 0)
    {
        TRACELOG(RL_E_LOG_WARNING, "GetMonitorPhysicalWidth() implemented for first monitor only");
    }
    else if ((platform.connector) && (platform.modeIndex >= 0))
    {
...
int RLGetMonitorPhysicalHeight(int monitor)
{
    ...
    if (monitor != 0)
    {
        TRACELOG(RL_E_LOG_WARNING, "GetMonitorPhysicalHeight() implemented for first monitor only");
    }
    else if ((platform.connector) && (platform.modeIndex >= 0))
    {
...
const char *RLGetMonitorName(int monitor)
{
    ...
    if (monitor != 0)
    {
        TRACELOG(RL_E_LOG_WARNING, "GetMonitorName() implemented for first monitor only");
    }
    else if ((platform.connector) && (platform.modeIndex >= 0))
    {
...
static void DestroyFrameBufferCallback(struct gbm_bo *bo, void *data)
{
    ...
    for (int i = 0; i < fbCacheCount; i++)
    {
        if (fbCache[i].bo == bo)
        {
            TRACELOG(RL_E_LOG_INFO, "DISPLAY: DRM: Framebuffer removed [%u]", (uintptr_t)fbId);
...
static uint32_t GetOrCreateFbForBo(struct gbm_bo *bo)
{
    ...
    TRACELOG(RL_E_LOG_INFO, "DISPLAY: DRM: Added new buffer object [%u]" , (uintptr_t)fbId);
...
int InitSwapScreenBuffer()
{
    ...
    {
        TRACELOG(RL_E_LOG_ERROR, "DISPLAY: DRM: Swap buffers can not be initialized");
        return -1;
    }
    ...
    if (!bo)
    {
        TRACELOG(RL_E_LOG_ERROR, "DISPLAY: DRM: Failed to lock initial swap buffer");
        return -1;
    }
    ...
    {
        TRACELOG(RL_E_LOG_ERROR, "DISPLAY: DRM: Failed to initialize CRTC setup. ERROR: %s", strerror(errno));
        gbm_surface_release_buffer(platform.gbmSurface, bo);
        return -1;
    }
...
void RLSwapScreenBuffer(void)
{
    ...
    if (!platform.gbmSurface || (-1 == platform.fd) || !platform.connector || !platform.crtc) TRACELOG(RL_E_LOG_ERROR, "DISPLAY: DRM initialization failed to swap");

    struct gbm_bo *bo = gbm_surface_lock_front_buffer(platform.gbmSurface);
    if (!bo) TRACELOG(RL_E_LOG_ERROR, "DISPLAY: Failed GBM to lock front buffer");
    ...
    if (result != 0) TRACELOG(RL_E_LOG_ERROR, "DISPLAY: drmModeAddFB() failed with result: %d", result);
    ...
    if (result != 0) TRACELOG(RL_E_LOG_ERROR, "DISPLAY: drmModeSetCrtc() failed with result: %d", result);

    if (platform.prevFB)
    {
        result = drmModeRmFB(platform.fd, platform.prevFB);
        if (result != 0) TRACELOG(RL_E_LOG_ERROR, "DISPLAY: drmModeRmFB() failed with result: %d", result);
    }
    ...
    {
        TRACELOG(RL_E_LOG_ERROR, "DISPLAY: DRM initialization failed to swap");
        return;
    }
    ...
    if (result < 0)
    {
        TRACELOG(RL_E_LOG_ERROR, "DISPLAY: Failed to create dumb buffer: %s", strerror(errno));
        return;
    }
    ...
    if (result != 0)
    {
        TRACELOG(RL_E_LOG_ERROR, "DISPLAY: drmModeAddFB() failed with result: %d (%s)", result, strerror(errno));
    ...
    if (result != 0)
    {
        TRACELOG(RL_E_LOG_ERROR, "DISPLAY: Failed to map dumb buffer: %s", strerror(errno));
    ...
    {
        TRACELOG(RL_E_LOG_ERROR, "DISPLAY: Failed to mmap dumb buffer: %s", strerror(errno));
        drmModeRmFB(platform.fd, fb);
        struct drm_mode_destroy_dumb dreq = { 0 };
        dreq.handle = creq.handle;
        drmIoctl(platform.fd, DRM_IOCTL_MODE_DESTROY_DUMB, &dreq);
    ...
    else
    {
        ...
        if (!res)
        {
            TRACELOG(RL_E_LOG_ERROR, "DISPLAY: Failed to get DRM resources");
            drmModeRmFB(platform.fd, fb);
            struct drm_mode_destroy_dumb dreq = { 0 };
            dreq.handle = creq.handle;
            drmIoctl(platform.fd, DRM_IOCTL_MODE_DESTROY_DUMB, &dreq);
        ...
        if (!crtcId)
        {
            TRACELOG(RL_E_LOG_ERROR, "DISPLAY: No compatible CRTC found");
            drmModeRmFB(platform.fd, fb);
            struct drm_mode_destroy_dumb dreq = { 0 };
            dreq.handle = creq.handle;
            drmIoctl(platform.fd, DRM_IOCTL_MODE_DESTROY_DUMB, &dreq);
    ...
    if (result != 0)
    {
        TRACELOG(RL_E_LOG_ERROR, "DISPLAY: drmModeSetCrtc() failed with result: %d (%s)", result, strerror(errno));
        TRACELOG(RL_E_LOG_ERROR, "DISPLAY: CRTC ID: %u, FB ID: %u, Connector ID: %u", crtcId, fb, platform.connector->connector_id);
        TRACELOG(RL_E_LOG_ERROR, "DISPLAY: Mode: %dx%d@%d", mode->hdisplay, mode->vdisplay, mode->vrefresh);
    ...
    {
        result = drmModeRmFB(platform.fd, platform.prevFB);
        if (result != 0) TRACELOG(RL_E_LOG_WARNING, "DISPLAY: drmModeRmFB() failed with result: %d", result);
    }
...
{
    TRACELOG(RL_E_LOG_WARNING, "OpenURL() not implemented on target platform");
}
...
int InitPlatform(void)
{
    ...
    if (platform.fd != -1) TRACELOG(RL_E_LOG_INFO, "DISPLAY: Default graphic device DRM opened successfully");
#else
    TRACELOG(RL_E_LOG_WARNING, "DISPLAY: No graphic card set, trying platform-gpu-card");
    platform.fd = open("/dev/dri/by-path/platform-gpu-card", O_RDWR); // VideoCore VI (Raspberry Pi 4)
    if (platform.fd != -1) TRACELOG(RL_E_LOG_INFO, "DISPLAY: platform-gpu-card opened successfully");

    drmModeRes *res = NULL;
    if ((platform.fd == -1) || ((res = drmModeGetResources(platform.fd)) == NULL))
    {
        if (platform.fd != -1) close(platform.fd);
        TRACELOG(RL_E_LOG_WARNING, "DISPLAY: Failed to open platform-gpu-card, trying card1");
        platform.fd = open("/dev/dri/card1", O_RDWR); // Other Embedded
        if (platform.fd != -1) TRACELOG(RL_E_LOG_INFO, "DISPLAY: card1 opened successfully");
    }

    if ((platform.fd == -1) || ((res = drmModeGetResources(platform.fd)) == NULL))
    {
        if (platform.fd != -1) close(platform.fd);
        TRACELOG(RL_E_LOG_WARNING, "DISPLAY: Failed to open graphic card1, trying card0");
        platform.fd = open("/dev/dri/card0", O_RDWR); // VideoCore IV (Raspberry Pi 1-3)
        if (platform.fd != -1) TRACELOG(RL_E_LOG_INFO, "DISPLAY: card0 opened successfully");
    }

    if ((platform.fd == -1) || ((res = drmModeGetResources(platform.fd)) == NULL))
    {
        if (platform.fd != -1) close(platform.fd);
        TRACELOG(RL_E_LOG_WARNING, "DISPLAY: Failed to open graphic card0, trying card2");
        platform.fd = open("/dev/dri/card2", O_RDWR);
        if (platform.fd != -1) TRACELOG(RL_E_LOG_INFO, "DISPLAY: card2 opened successfully");
    }
#endif

    if (platform.fd == -1)
    {
        TRACELOG(RL_E_LOG_WARNING, "DISPLAY: Failed to open graphic card");
        return -1;
    }

    if (!res)
    {
        TRACELOG(RL_E_LOG_WARNING, "DISPLAY: Failed get DRM resources");
        close(platform.fd);
        return -1;
    }

    TRACELOG(RL_E_LOG_TRACE, "DISPLAY: Connectors found: %i", res->count_connectors);

    // Connector detection
    for (size_t i = 0; i < res->count_connectors; i++)
    {
        TRACELOG(RL_E_LOG_TRACE, "DISPLAY: Connector index %i", i);

        drmModeConnector *con = drmModeGetConnector(platform.fd, res->connectors[i]);
        if (!con)
        {
            TRACELOG(RL_E_LOG_WARNING, "DISPLAY: Failed to get connector %i", i);
            continue;
        }

        TRACELOG(RL_E_LOG_TRACE, "DISPLAY: Connector %i modes detected: %i", i, con->count_modes);
        TRACELOG(RL_E_LOG_TRACE, "DISPLAY: Connector %i status: %s", i,
        ...
        if (((con->connection == DRM_MODE_CONNECTED) || (con->connection == DRM_MODE_UNKNOWNCONNECTION)) && (con->count_modes > 0))
        {
            ...
            if (con->encoder_id)
            {
                TRACELOG(RL_E_LOG_TRACE, "DISPLAY: DRM connector %i connected with encoder", i);
                platform.connector = con;
                break;
            }
            else TRACELOG(RL_E_LOG_TRACE, "DISPLAY: DRM connector %i connected but no encoder", i);
#else
            // For software rendering, we can accept even without encoder_id
            TRACELOG(RL_E_LOG_TRACE, "DISPLAY: DRM connector %i suitable for software rendering", i);
            platform.connector = con;
            break;
#endif
        }
        ...
        {
            TRACELOG(RL_E_LOG_TRACE, "DISPLAY: DRM connector %i NOT suitable (deleting)", i);
    ...
    if (!platform.connector)
    {
        TRACELOG(RL_E_LOG_WARNING, "DISPLAY: No suitable DRM connector found");
        drmModeFreeResources(res);
        close(platform.fd);
        return -1;
    }
    ...
    if (!enc)
    {
        TRACELOG(RL_E_LOG_WARNING, "DISPLAY: Failed to get DRM mode encoder");
    ...
    if (!platform.crtc)
    {
        TRACELOG(RL_E_LOG_WARNING, "DISPLAY: Failed to get DRM mode crtc");
    ...
    {
        TRACELOG(RL_E_LOG_TRACE, "DISPLAY: Selecting DRM connector mode for current used mode...");

        platform.modeIndex = FindMatchingConnectorMode(platform.connector, &platform.crtc->mode);

        if (platform.modeIndex < 0)
        {
            TRACELOG(RL_E_LOG_WARNING, "DISPLAY: No matching DRM connector mode found");
            drmModeFreeEncoder(enc);
            drmModeFreeConnector(platform.connector);
            drmModeFreeResources(res);
            close(platform.fd);
    ...
    const bool allowInterlaced = FLAG_IS_SET(CORE.Window.flags, RL_E_FLAG_INTERLACED_HINT);
    ...
    if (platform.modeIndex < 0)
    {
        TRACELOG(RL_E_LOG_WARNING, "DISPLAY: Failed to find a suitable DRM connector mode");
        drmModeFreeEncoder(enc);
        drmModeFreeConnector(platform.connector);
        drmModeFreeResources(res);
        close(platform.fd);
    ...
    TRACELOG(RL_E_LOG_INFO, "DISPLAY: Selected DRM connector mode %s (%ux%u%c@%u)", platform.connector->modes[platform.modeIndex].name,
    ...
    if (platform.connector->count_modes > 0)
    {
        ...
        TRACELOG(RL_E_LOG_INFO, "DISPLAY: Selected DRM connector mode %s (%ux%u%c@%u) for software rendering",
    ...
    {
        TRACELOG(RL_E_LOG_WARNING, "DISPLAY: No modes available for connector");
        drmModeFreeConnector(platform.connector);
        drmModeFreeResources(res);
        close(platform.fd);
        return -1;
    ...
    {
        TRACELOG(RL_E_LOG_WARNING, "DISPLAY: Failed to create GBM device");
        return -1;
    }
    ...
    {
        TRACELOG(RL_E_LOG_WARNING, "DISPLAY: Failed to create GBM surface");
        return -1;
    }
    ...
    {
        TRACELOG(RL_E_LOG_WARNING, "DISPLAY: Failed to get EGL config count: 0x%x", eglGetError());
        return -1;
    }

    TRACELOG(RL_E_LOG_TRACE, "DISPLAY: EGL configs available: %d", numConfigs);

    EGLConfig *configs = (EGLConfig *)RL_CALLOC(numConfigs, sizeof(*configs));
    if (!configs)
    {
        TRACELOG(RL_E_LOG_WARNING, "DISPLAY: Failed to get memory for EGL configs");
        return -1;
    }
    ...
    {
        TRACELOG(RL_E_LOG_WARNING, "DISPLAY: Failed to choose EGL config: 0x%x", eglGetError());
        RL_FREE(configs);
        return -1;
    }

    TRACELOG(RL_E_LOG_TRACE, "DISPLAY: EGL matching configs available: %d", matchingNumConfigs);
    ...
    for (EGLint i = 0; i < matchingNumConfigs; i++)
    {
        ...
        {
            TRACELOG(RL_E_LOG_WARNING, "DISPLAY: Failed to get EGL config attribute: 0x%x", eglGetError());
            continue;
        }

        if (GBM_FORMAT_ARGB8888 == id)
        {
            TRACELOG(RL_E_LOG_TRACE, "DISPLAY: Using EGL config: %d", i);
    ...
    if (!found)
    {
        TRACELOG(RL_E_LOG_WARNING, "DISPLAY: Failed to find a suitable EGL config");
        return -1;
    }
    ...
    if (platform.surface == EGL_NO_SURFACE)
    {
        TRACELOG(RL_E_LOG_WARNING, "DISPLAY: Failed to create EGL window surface: 0x%04x", eglGetError());
        return -1;
    }
    ...
    TRACELOG(RL_E_LOG_INFO, "DISPLAY: Device initialized successfully (Software Rendering)");
    TRACELOG(RL_E_LOG_INFO, "    > Display size: %i x %i", CORE.Window.display.width, CORE.Window.display.height);
    TRACELOG(RL_E_LOG_INFO, "    > Screen size:  %i x %i", CORE.Window.screen.width, CORE.Window.screen.height);
    TRACELOG(RL_E_LOG_INFO, "    > Render size:  %i x %i", CORE.Window.render.width, CORE.Window.render.height);
    TRACELOG(RL_E_LOG_INFO, "    > Viewport offsets: %i, %i", CORE.Window.renderOffset.x, CORE.Window.renderOffset.y);
    ...
    {
        TRACELOG(RL_E_LOG_FATAL, "PLATFORM: Failed to initialize graphic device");
        return -1;
    }
    else RLSetWindowPosition(RLGetMonitorWidth(RLGetCurrentMonitor())/2 - CORE.Window.screen.width/2, RLGetMonitorHeight(RLGetCurrentMonitor())/2 - CORE.Window.screen.height/2);
    ...
    {
        TRACELOG(RL_E_LOG_INFO, "PLATFORM: DRM: Initialized successfully");
        return 0;
    }
    else
    {
        TRACELOG(RL_E_LOG_INFO, "PLATFORM: DRM: Initialized failed");
        return -1;
    }
#else // !SUPPORT_DRM_CACHE
    TRACELOG(RL_E_LOG_INFO, "PLATFORM: DRM: Initialized successfully");
...
static void InitKeyboard(void)
{
    ...
    if (result < 0) TRACELOG(RL_E_LOG_WARNING, "DRM: Failed to change keyboard mode, an SSH keyboard is probably used");
    else
    {
...
static void InitEvdevInput(void)
{
    ...
    else TRACELOG(RL_E_LOG_WARNING, "INPUT: Failed to open linux event directory: %s", DEFAULT_EVDEV_PATH);
}
...
static void ConfigureEvdevDevice(char *device)
{
    ...
    if (fd < 0)
    {
        TRACELOG(RL_E_LOG_WARNING, "SYSTEM: Failed to open input device: %s", device);
        return;
    }
    ...
    if (isMouse || isTouch)
    {
        ...
        if (prioritize)
        {
            ...
            if (platform.mouseFd != -1)
            {
                TRACELOG(RL_E_LOG_INFO, "INPUT: Overwriting previous input device with new %s", deviceKindStr);
                close(platform.mouseFd);
            }
            ...
            TRACELOG(RL_E_LOG_INFO, "INPUT: Initialized input device %s as %s", device, deviceKindStr);
        }
        else
        {
            TRACELOG(RL_E_LOG_INFO, "INPUT: Ignoring device %s (keeping higher priority %s device)", device, platform.mouseIsTouch ? "touchscreen" : "mouse");
            close(fd);
            return;
        }
    }
    ...
    while (read(fd, &event, sizeof(event)) == (int)sizeof(event))
    {
        ...
        if ((event.code >= 1) && (event.code <= 255))
        {
            ...
            if ((keycode > 0) && (keycode < MAX_KEYBOARD_KEYS))
            {
                ...
                TRACELOG(RL_E_LOG_DEBUG, "INPUT: KEY_%s Keycode(linux): %4i KeyCode(raylib): %4i", (event.value == 0)? "UP  " : "DOWN", event.code, keycode);
            }
        }
    }
}
        ...
        while (read(platform.gamepadStreamFd[i], &event, sizeof(event)) == (int)sizeof(event))
        {
            if (event.type == EV_KEY)
            {
                if (event.code < KEYMAP_SIZE)
                {
                    ...
                    TRACELOG(RL_E_LOG_DEBUG, "INPUT: Gamepad %2i: KEY_%s Keycode(linux): %4i Keycode(raylib): %4i", i, (event.value == 0)? "UP" : "DOWN", event.code, keycode);

                    if ((keycode != 0) && (keycode < MAX_GAMEPAD_BUTTONS))
                    {
                        // 1 - button pressed, 0 - button released
                        CORE.Input.Gamepad.currentButtonState[i][keycode] = event.value;
                        CORE.Input.Gamepad.lastButtonPressed = (event.value == 1)? keycode : RL_E_GAMEPAD_BUTTON_UNKNOWN;
                    }
                }
            }
            else if (event.type == EV_ABS)
            {
                if (event.code < ABS_CNT)
                {
                    ...
                    TRACELOG(RL_E_LOG_DEBUG, "INPUT: Gamepad %2i: Axis: %2i Value: %i", i, axisRaylib, event.value);
...
static void PollMouseEvents(void)
{
    ...
    while (read(fd, &event, sizeof(event)) == (int)sizeof(event))
    {
        ...
        if (event.type == EV_ABS)
        {
            ...
            {
                int previousMouseLeftButtonState = platform.currentButtonStateEvdev[RL_E_MOUSE_BUTTON_LEFT];

                if (!event.value && previousMouseLeftButtonState)
                {
                    platform.currentButtonStateEvdev[RL_E_MOUSE_BUTTON_LEFT] = 0;
                    if (touchAction != 1) touchAction = 0; // TOUCH_ACTION_UP
                }

                if (event.value && !previousMouseLeftButtonState)
                {
                    platform.currentButtonStateEvdev[RL_E_MOUSE_BUTTON_LEFT] = 1;
                    touchAction = 1; // TOUCH_ACTION_DOWN
                }
            }
        }
        ...
        if (event.type == EV_KEY)
        {
            ...
            {
                platform.currentButtonStateEvdev[RL_E_MOUSE_BUTTON_LEFT] = event.value;
            ...
            if (event.code == BTN_RIGHT) platform.currentButtonStateEvdev[RL_E_MOUSE_BUTTON_RIGHT] = event.value;
            if (event.code == BTN_MIDDLE) platform.currentButtonStateEvdev[RL_E_MOUSE_BUTTON_MIDDLE] = event.value;
            if (event.code == BTN_SIDE) platform.currentButtonStateEvdev[RL_E_MOUSE_BUTTON_SIDE] = event.value;
            if (event.code == BTN_EXTRA) platform.currentButtonStateEvdev[RL_E_MOUSE_BUTTON_EXTRA] = event.value;
            if (event.code == BTN_FORWARD) platform.currentButtonStateEvdev[RL_E_MOUSE_BUTTON_FORWARD] = event.value;
            if (event.code == BTN_BACK) platform.currentButtonStateEvdev[RL_E_MOUSE_BUTTON_BACK] = event.value;
        }
...
static int FindMatchingConnectorMode(const drmModeConnector *connector, const drmModeModeInfo *mode)
{
    ...
    {
        TRACELOG(RL_E_LOG_TRACE, "DISPLAY: DRM mode: %d %ux%u@%u %s", i, connector->modes[i].hdisplay, connector->modes[i].vdisplay,
...
{
    TRACELOG(RL_E_LOG_TRACE, "DISPLAY: Searching exact connector mode for %ux%u@%u, selecting an interlaced mode is allowed: %s", width, height, fps, allowInterlaced? "yes" : "no");
    ...
    for (int i = 0; i < platform.connector->count_modes; i++)
    {
        ...
        TRACELOG(RL_E_LOG_TRACE, "DISPLAY: DRM Mode %d %ux%u@%u %s", i, mode->hdisplay, mode->vdisplay, mode->vrefresh, (FLAG_IS_SET(mode->flags, DRM_MODE_FLAG_INTERLACE) > 0)? "interlaced" : "progressive");
    ...
    TRACELOG(RL_E_LOG_TRACE, "DISPLAY: No DRM exact matching mode found");
    return -1;
}
...
{
    TRACELOG(RL_E_LOG_TRACE, "DISPLAY: Searching nearest connector mode for %ux%u@%u, selecting an interlaced mode is allowed: %s", width, height, fps, allowInterlaced? "yes" : "no");
    ...
    for (int i = 0; i < platform.connector->count_modes; i++)
    {
        ...
        TRACELOG(RL_E_LOG_TRACE, "DISPLAY: DRM mode: %d %ux%u@%u %s", i, mode->hdisplay, mode->vdisplay, mode->vrefresh,
            (FLAG_IS_SET(mode->flags, DRM_MODE_FLAG_INTERLACE) > 0)? "interlaced" : "progressive");

        if ((mode->hdisplay < width) || (mode->vdisplay < height))
        {
            TRACELOG(RL_E_LOG_TRACE, "DISPLAY: DRM mode is too small");
            continue;
        }

        if ((FLAG_IS_SET(mode->flags, DRM_MODE_FLAG_INTERLACE) > 0) && !allowInterlaced)
        {
            TRACELOG(RL_E_LOG_TRACE, "DISPLAY: DRM shouldn't choose an interlaced mode");
            continue;
        }
