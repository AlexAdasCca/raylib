...
// Image fileformats not supported by default
    ...
    #define RL_GPUTEX_LOG(...) TRACELOG(RL_E_LOG_WARNING, "IMAGE: " __VA_ARGS__)
...
static RLVector4 *LoadImageDataNormalized(RLImage image);       // Load pixel data from image as Vector4 array (float normalized)

//----------------------------------------------------------------------------------
// Module Functions Definition
//----------------------------------------------------------------------------------
// Load image from file into CPU memory (RAM)
RLImage RLLoadImage(const char *fileName)
{
    RLImage image = { 0 };
...
RLImage RLLoadImageRaw(const char *fileName, int width, int height, int format, int headerSize)
{
    RLImage image = { 0 };
...
RLImage RLLoadImageAnim(const char *fileName, int *frames)
{
    RLImage image = { 0 };
    int frameCount = 0;
...
//  - All frames are returned in RGBA format
//  - Frames delay data is discarded
RLImage RLLoadImageAnimFromMemory(const char *fileType, const unsigned char *fileData, int dataSize, int *frames)
{
    RLImage image = { 0 };
    int frameCount = 0;
...
RLImage RLLoadImageFromMemory(const char *fileType, const unsigned char *fileData, int dataSize)
{
    RLImage image = { 0 };
    ...
    {
        TRACELOG(RL_E_LOG_WARNING, "IMAGE: Invalid file data");
        return image;
    }
    if (fileType == NULL)
    {
        TRACELOG(RL_E_LOG_WARNING, "IMAGE: Missing file extension");
        return image;
    }
        ...
        )
    {
        ...
        if (fileData != NULL)
        {
            ...
            if (image.data != NULL)
            {
                ...
                if (comp == 1) image.format = RL_E_PIXELFORMAT_UNCOMPRESSED_GRAYSCALE;
                else if (comp == 2) image.format = RL_E_PIXELFORMAT_UNCOMPRESSED_GRAY_ALPHA;
                else if (comp == 3) image.format = RL_E_PIXELFORMAT_UNCOMPRESSED_R8G8B8;
                else if (comp == 4) image.format = RL_E_PIXELFORMAT_UNCOMPRESSED_R8G8B8A8;
            }
        }
#endif
    }
    ...
    else if ((strcmp(fileType, ".hdr") == 0) || (strcmp(fileType, ".HDR") == 0))
    {
        ...
        if (fileData != NULL)
        {
            ...
            if (comp == 1) image.format = RL_E_PIXELFORMAT_UNCOMPRESSED_R32;
            else if (comp == 3) image.format = RL_E_PIXELFORMAT_UNCOMPRESSED_R32G32B32;
            else if (comp == 4) image.format = RL_E_PIXELFORMAT_UNCOMPRESSED_R32G32B32A32;
            else
            {
                TRACELOG(RL_E_LOG_WARNING, "IMAGE: HDR file format not supported");
                RLUnloadImage(image);
            }
        }
#endif
    ...
    else if ((strcmp(fileType, ".qoi") == 0) || (strcmp(fileType, ".QOI") == 0))
    {
        if (fileData != NULL)
        {
            ...
            image.format = (desc.channels == 4)? RL_E_PIXELFORMAT_UNCOMPRESSED_R8G8B8A8 : RL_E_PIXELFORMAT_UNCOMPRESSED_R8G8B8;
            image.mipmaps = 1;
        }
    }
#endif
...
#endif
    else TRACELOG(RL_E_LOG_WARNING, "IMAGE: Data format not supported");

    if (image.data != NULL) TRACELOG(RL_E_LOG_INFO, "IMAGE: Data loaded successfully (%ix%i | %s | %i mipmaps)", image.width, image.height, rlGetPixelFormatName(image.format), image.mipmaps);
    else TRACELOG(RL_E_LOG_WARNING, "IMAGE: Failed to load image data");
...
RLImage RLLoadImageFromTexture(RLTexture2D texture)
{
    RLImage image = { 0 };

    if (texture.format < RL_E_PIXELFORMAT_COMPRESSED_DXT1_RGB)
    {
        ...
        if (image.data != NULL)
        {
            ...
            image.format = RL_E_PIXELFORMAT_UNCOMPRESSED_R8G8B8A8;
#endif
            TRACELOG(RL_E_LOG_INFO, "TEXTURE: [ID %i] Pixel data retrieved successfully", texture.id);
        }
        else TRACELOG(RL_E_LOG_WARNING, "TEXTURE: [ID %i] Failed to retrieve pixel data", texture.id);
    }
    else TRACELOG(RL_E_LOG_WARNING, "TEXTURE: [ID %i] Failed to retrieve compressed pixel data", texture.id);

    return image;
}

// Load image from screen buffer and (screenshot)
RLImage RLLoadImageFromScreen(void)
{
    RLImage image = { 0 };

    image.width = (int)(RLGetRenderWidth());
    image.height = (int)(RLGetRenderHeight());
    image.mipmaps = 1;
    image.format = RL_E_PIXELFORMAT_UNCOMPRESSED_R8G8B8A8;
...
bool RLIsImageValid(RLImage image)
{
    bool result = false;
...
void RLUnloadImage(RLImage image)
{
    RL_FREE(image.data);
}
...
bool RLExportImage(RLImage image, const char *fileName)
{
    int result = 0;
    ...
    if (image.format == RL_E_PIXELFORMAT_UNCOMPRESSED_GRAYSCALE) channels = 1;
    else if (image.format == RL_E_PIXELFORMAT_UNCOMPRESSED_GRAY_ALPHA) channels = 2;
    else if (image.format == RL_E_PIXELFORMAT_UNCOMPRESSED_R8G8B8) channels = 3;
    else if (image.format == RL_E_PIXELFORMAT_UNCOMPRESSED_R8G8B8A8) channels = 4;
    else
    {
...
#if defined(SUPPORT_FILEFORMAT_PNG)
    else if (RLIsFileExtension(fileName, ".png"))
    {
        int dataSize = 0;
    ...
    {
        channels = 0;
        if (image.format == RL_E_PIXELFORMAT_UNCOMPRESSED_R8G8B8) channels = 3;
        else if (image.format == RL_E_PIXELFORMAT_UNCOMPRESSED_R8G8B8A8) channels = 4;
        else TRACELOG(RL_E_LOG_WARNING, "IMAGE: Image pixel format must be R8G8B8 or R8G8B8A8");
    ...
    else TRACELOG(RL_E_LOG_WARNING, "IMAGE: Export image format requested not supported");

    if (allocatedData) RL_FREE(imgData);
#endif      // SUPPORT_IMAGE_EXPORT

    if (result != 0) TRACELOG(RL_E_LOG_INFO, "FILEIO: [%s] Image exported successfully", fileName);
    else TRACELOG(RL_E_LOG_WARNING, "FILEIO: [%s] Failed to export image", fileName);

    return result;
}

// Export image to memory buffer
unsigned char *RLExportImageToMemory(RLImage image, const char *fileType, int *dataSize)
{
...
bool RLExportImageAsCode(RLImage image, const char *fileName)
{
    bool success = false;
    ...
    if (success != 0) TRACELOG(RL_E_LOG_INFO, "FILEIO: [%s] Image as code exported successfully", fileName);
    else TRACELOG(RL_E_LOG_WARNING, "FILEIO: [%s] Failed to export image as code", fileName);
...
RLImage RLGenImageCellular(int width, int height, int tileSize)
{
    ...
    for (int y = 0; y < height; y++)
    {
        ...
        for (int x = 0; x < width; x++)
        {
            ...
            pixels[y*width + x] = (RLColor){ intensityUC, intensityUC, intensityUC, 255 };
        }
    }
...
RLImage RLGenImageText(int width, int height, const char *text)
{
    RLImage image = { 0 };
    ...
    image.width = width;
    image.height = height;
    image.format = RL_E_PIXELFORMAT_UNCOMPRESSED_GRAYSCALE;
...
RLImage RLImageCopy(RLImage image)
{
    RLImage newImage = { 0 };
...
RLImage RLImageFromImage(RLImage image, RLRectangle rec)
{
    RLImage result = { 0 };
...
void RLImageCrop(RLImage *image, RLRectangle crop)
{
    ...
    {
        TRACELOG(RL_E_LOG_WARNING, "IMAGE: Failed to crop, rectangle out of bounds");
        return;
    }

    if (image->mipmaps > 1) TRACELOG(RL_E_LOG_WARNING, "Image manipulation only applied to base mipmap level");
    if (image->format >= RL_E_PIXELFORMAT_COMPRESSED_DXT1_RGB) TRACELOG(RL_E_LOG_WARNING, "Image manipulation not supported for compressed formats");
    else
    {
        int bytesPerPixel = RLGetPixelDataSize(1, 1, image->format);
...
RLImage RLImageText(const char *text, int fontSize, RLColor color)
{
    RLImage imText = { 0 };
    ...
    TRACELOG(RL_E_LOG_WARNING, "IMAGE: ImageTextEx() requires module: rtext");
#endif
    return imText;
}
...
RLImage RLImageTextEx(RLFont font, const char *text, float fontSize, float spacing, RLColor tint)
{
    RLImage imText = { 0 };
#if defined(SUPPORT_MODULE_RTEXT)
    ...
    RLVector2 imSize = RLMeasureTextEx(font, text, (float)font.baseSize, spacing);  // WARNING: Module required: rtext
    RLVector2 textSize = RLMeasureTextEx(font, text, fontSize, spacing);
    ...
    for (int i = 0; i < textLength;)
    {
        ...
        else
        {
            if ((codepoint != ' ') && (codepoint != '\t'))
            {
                ...
                RLImageDraw(&imText, font.glyphs[index].image, (RLRectangle){ 0, 0, (float)font.glyphs[index].image.width, (float)font.glyphs[index].image.height }, rec, tint);
            }
    ...
    if (textSize.y != imSize.y)
    {
        ...
        TRACELOG(RL_E_LOG_INFO, "IMAGE: Text scaled by factor: %f", scaleFactor);
...
RLImage RLImageFromChannel(RLImage image, int selectedChannel)
{
    RLImage result = { 0 };
    ...
    {
        TRACELOG(RL_E_LOG_WARNING, "Channel cannot be negative. Setting channel to 0.");
        selectedChannel = 0;
    }

    if (image.format == RL_E_PIXELFORMAT_UNCOMPRESSED_GRAYSCALE ||
        image.format == RL_E_PIXELFORMAT_UNCOMPRESSED_R32 ||
        image.format == RL_E_PIXELFORMAT_UNCOMPRESSED_R16)
    {
        if (selectedChannel > 0)
        {
            TRACELOG(RL_E_LOG_WARNING, "This image has only 1 channel. Setting channel to it.");
            selectedChannel = 0;
        }
    }
    else if (image.format == RL_E_PIXELFORMAT_UNCOMPRESSED_GRAY_ALPHA)
    {
        if (selectedChannel > 1)
        {
            TRACELOG(RL_E_LOG_WARNING, "This image has only 2 channels. Setting channel to alpha.");
            selectedChannel = 1;
        }
    }
    else if (image.format == RL_E_PIXELFORMAT_UNCOMPRESSED_R5G6B5 ||
             image.format == RL_E_PIXELFORMAT_UNCOMPRESSED_R8G8B8 ||
             image.format == RL_E_PIXELFORMAT_UNCOMPRESSED_R32G32B32 ||
             image.format == RL_E_PIXELFORMAT_UNCOMPRESSED_R16G16B16)
    {
        if (selectedChannel > 2)
        {
            TRACELOG(RL_E_LOG_WARNING, "This image has only 3 channels. Setting channel to red.");
            selectedChannel = 0;
        }
    }
    ...
    {
        TRACELOG(RL_E_LOG_WARNING, "ImageFromChannel supports channels 0 to 3 (rgba). Setting channel to alpha.");
        selectedChannel = 3;
    }

    // TODO: Consider other one-channel formats: R16, R32
    result.format = RL_E_PIXELFORMAT_UNCOMPRESSED_GRAYSCALE;
...
void RLImageResizeNN(RLImage *image, int newWidth, int newHeight)
{
    // Security check to avoid program crash
    if ((image->data == NULL) || (image->width == 0) || (image->height == 0)) return;
...
void RLImageResize(RLImage *image, int newWidth, int newHeight)
{
    // Security check to avoid program crash
    if ((image->data == NULL) || (image->width == 0) || (image->height == 0)) return;
        ...
        (image->format == RL_E_PIXELFORMAT_UNCOMPRESSED_R8G8B8A8))
    {
        ...
        switch (image->format)
        {
            case RL_E_PIXELFORMAT_UNCOMPRESSED_GRAYSCALE: stbir_resize_uint8_linear((unsigned char *)image->data, image->width, image->height, 0, output, newWidth, newHeight, 0, (stbir_pixel_layout)1); break;
            case RL_E_PIXELFORMAT_UNCOMPRESSED_GRAY_ALPHA: stbir_resize_uint8_linear((unsigned char *)image->data, image->width, image->height, 0, output, newWidth, newHeight, 0, (stbir_pixel_layout)2); break;
            case RL_E_PIXELFORMAT_UNCOMPRESSED_R8G8B8: stbir_resize_uint8_linear((unsigned char *)image->data, image->width, image->height, 0, output, newWidth, newHeight, 0, (stbir_pixel_layout)3); break;
            case RL_E_PIXELFORMAT_UNCOMPRESSED_R8G8B8A8: stbir_resize_uint8_linear((unsigned char *)image->data, image->width, image->height, 0, output, newWidth, newHeight, 0, (stbir_pixel_layout)4); break;
            default: break;
        }
...
void RLImageResizeCanvas(RLImage *image, int newWidth, int newHeight, int offsetX, int offsetY, RLColor fill)
{
    // Security check to avoid program crash
    if ((image->data == NULL) || (image->width == 0) || (image->height == 0)) return;
...
void RLImageToPOT(RLImage *image, RLColor fill)
{
    // Security check to avoid program crash
    if ((image->data == NULL) || (image->width == 0) || (image->height == 0)) return;
...
void RLImageAlphaCrop(RLImage *image, float threshold)
{
    // Security check to avoid program crash
    if ((image->data == NULL) || (image->width == 0) || (image->height == 0)) return;

    RLRectangle crop = RLGetImageAlphaBorder(*image, threshold);
...
void RLImageAlphaClear(RLImage *image, RLColor color, float threshold)
{
    // Security check to avoid program crash
    if ((image->data == NULL) || (image->width == 0) || (image->height == 0)) return;
...
void RLImageAlphaMask(RLImage *image, RLImage alphaMask)
{
    if ((image->width != alphaMask.width) || (image->height != alphaMask.height))
    {
        TRACELOG(RL_E_LOG_WARNING, "IMAGE: Alpha mask must be same size as image");
    }
    else if (image->format >= RL_E_PIXELFORMAT_COMPRESSED_DXT1_RGB)
    {
        TRACELOG(RL_E_LOG_WARNING, "IMAGE: Alpha mask can not be applied to compressed data formats");
    }
    else
    {
...
void RLImageAlphaPremultiply(RLImage *image)
{
    // Security check to avoid program crash
    if ((image->data == NULL) || (image->width == 0) || (image->height == 0)) return;
...
void RLImageBlurGaussian(RLImage *image, int blurSize)
{
    // Security check to avoid program crash
    if ((image->data == NULL) || (image->width == 0) || (image->height == 0)) return;
...
void RLImageKernelConvolution(RLImage *image, const float *kernel, int kernelSize)
{
    ...
    {
        TRACELOG(RL_E_LOG_WARNING, "IMAGE: Convolution kernel must be square to be applied");
        return;
    }
...
void RLImageMipmaps(RLImage *image)
{
    // Security check to avoid program crash
    if ((image->data == NULL) || (image->width == 0) || (image->height == 0)) return;
    ...
    while ((mipWidth != 1) || (mipHeight != 1))
    {
        ...
        TRACELOG(RL_E_LOG_DEBUG, "IMAGE: Next mipmap level: %i x %i - current size %i", mipWidth, mipHeight, mipSize);
    ...
    if (image->mipmaps < mipCount)
    {
        ...
        RLImage imCopy = RLImageCopy(*image);
    ...
    else TRACELOG(RL_E_LOG_WARNING, "IMAGE: Mipmaps already available");
}

// Dither image data to 16bpp or lower (Floyd-Steinberg dithering)
// NOTE: In case selected bpp do not represent a known 16bit format,
// dithered data is stored in the LSB part of the unsigned short
void RLImageDither(RLImage *image, int rBpp, int gBpp, int bBpp, int aBpp)
{
    // Security check to avoid program crash
    if ((image->data == NULL) || (image->width == 0) || (image->height == 0)) return;

    if (image->format >= RL_E_PIXELFORMAT_COMPRESSED_DXT1_RGB)
    {
        TRACELOG(RL_E_LOG_WARNING, "IMAGE: Compressed data formats can not be dithered");
        return;
    }

    if ((rBpp + gBpp + bBpp + aBpp) > 16)
    {
        TRACELOG(RL_E_LOG_WARNING, "IMAGE: Unsupported dithering bpps (%ibpp), only 16bpp or lower modes supported", (rBpp+gBpp+bBpp+aBpp));
    }
    else
    {
        RLColor *pixels = RLLoadImageColors(*image);
        ...
        {
            TRACELOG(RL_E_LOG_WARNING, "IMAGE: Format is already 16bpp or lower, dithering could have no effect");
        }
...
void RLImageFlipVertical(RLImage *image)
{
    // Security check to avoid program crash
    if ((image->data == NULL) || (image->width == 0) || (image->height == 0)) return;
...
void RLImageFlipHorizontal(RLImage *image)
{
    // Security check to avoid program crash
    if ((image->data == NULL) || (image->width == 0) || (image->height == 0)) return;
...
void RLImageRotate(RLImage *image, int degrees)
{
    // Security check to avoid program crash
    if ((image->data == NULL) || (image->width == 0) || (image->height == 0)) return;
...
void RLImageRotateCW(RLImage *image)
{
    // Security check to avoid program crash
    if ((image->data == NULL) || (image->width == 0) || (image->height == 0)) return;
...
void RLImageRotateCCW(RLImage *image)
{
    // Security check to avoid program crash
    if ((image->data == NULL) || (image->width == 0) || (image->height == 0)) return;
...
void RLImageColorTint(RLImage *image, RLColor color)
{
    // Security check to avoid program crash
    if ((image->data == NULL) || (image->width == 0) || (image->height == 0)) return;
...
void RLImageColorInvert(RLImage *image)
{
    // Security check to avoid program crash
    if ((image->data == NULL) || (image->width == 0) || (image->height == 0)) return;
...
void RLImageColorGrayscale(RLImage *image)
{
    RLImageFormat(image, RL_E_PIXELFORMAT_UNCOMPRESSED_GRAYSCALE);
}

// Modify image color: contrast
// NOTE: Contrast values between -100 and 100
void RLImageColorContrast(RLImage *image, float contrast)
{
    // Security check to avoid program crash
    if ((image->data == NULL) || (image->width == 0) || (image->height == 0)) return;
...
void RLImageColorBrightness(RLImage *image, int brightness)
{
    // Security check to avoid program crash
    if ((image->data == NULL) || (image->width == 0) || (image->height == 0)) return;
...
void RLImageColorReplace(RLImage *image, RLColor color, RLColor replace)
{
    // Security check to avoid program crash
    if ((image->data == NULL) || (image->width == 0) || (image->height == 0)) return;
    ...
    if ((format == RL_E_PIXELFORMAT_UNCOMPRESSED_R8G8B8) ||
        (format == RL_E_PIXELFORMAT_UNCOMPRESSED_R5G6B5) ||
        (format == RL_E_PIXELFORMAT_UNCOMPRESSED_GRAYSCALE) ||
        (format == RL_E_PIXELFORMAT_UNCOMPRESSED_R32G32B32) ||
        (format == RL_E_PIXELFORMAT_UNCOMPRESSED_R16G16B16) ||
...
RLColor *RLLoadImageColors(RLImage image)
{
    ...
    {
        if ((image.format == RL_E_PIXELFORMAT_UNCOMPRESSED_R32) ||
            (image.format == RL_E_PIXELFORMAT_UNCOMPRESSED_R32G32B32) ||
            (image.format == RL_E_PIXELFORMAT_UNCOMPRESSED_R32G32B32A32)) TRACELOG(RL_E_LOG_WARNING, "IMAGE: Pixel format converted from 32bit to 8bit per channel");

        if ((image.format == RL_E_PIXELFORMAT_UNCOMPRESSED_R16) ||
            (image.format == RL_E_PIXELFORMAT_UNCOMPRESSED_R16G16B16) ||
            (image.format == RL_E_PIXELFORMAT_UNCOMPRESSED_R16G16B16A16)) TRACELOG(RL_E_LOG_WARNING, "IMAGE: Pixel format converted from 16bit to 8bit per channel");
...
RLColor *RLLoadImagePalette(RLImage image, int maxPaletteSize, int *colorCount)
{
            ...
            if (pixels[i].a > 0)
            {
                ...
                if (!colorInPalette)
                {
                    ...
                    if (palCount >= maxPaletteSize)
                    {
                        ...
                        TRACELOG(RL_E_LOG_WARNING, "IMAGE: Palette is greater than %i colors", maxPaletteSize);
                    }
                }
            }
        }
...
void RLUnloadImageColors(RLColor *colors)
{
    RL_FREE(colors);
}

// Unload colors palette loaded with LoadImagePalette()
void RLUnloadImagePalette(RLColor *colors)
{
    RL_FREE(colors);
}
...
// NOTE: Threshold is defined as a percentage: 0.0f -> 1.0f
RLRectangle RLGetImageAlphaBorder(RLImage image, float threshold)
{
    RLRectangle crop = { 0 };
...
RLColor RLGetImageColor(RLImage image, int x, int y)
{
    RLColor color = { 0 };
    ...
    if ((x >=0) && (x < image.width) && (y >= 0) && (y < image.height))
    {
        switch (image.format)
        {
            ...
            default: TRACELOG(RL_E_LOG_WARNING, "Compressed image format does not support color reading"); break;
        }
    }
    else TRACELOG(RL_E_LOG_WARNING, "Requested image pixel (%i, %i) out of bounds", x, y);
...
void RLImageClearBackground(RLImage *dst, RLColor color)
{
    // Security check to avoid program crash
...
void RLImageDrawPixel(RLImage *dst, int x, int y, RLColor color)
{
    // Security check to avoid program crash
...
void RLImageDrawPixelV(RLImage *dst, RLVector2 position, RLColor color)
{
    RLImageDrawPixel(dst, (int)position.x, (int)position.y, color);
}

// Draw line within an image
void RLImageDrawLine(RLImage *dst, int startPosX, int startPosY, int endPosX, int endPosY, RLColor color)
{
...
void RLImageDrawLineV(RLImage *dst, RLVector2 start, RLVector2 end, RLColor color)
{
...
void RLImageDrawLineEx(RLImage *dst, RLVector2 start, RLVector2 end, int thick, RLColor color)
{
    // Round start and end positions to nearest integer coordinates
    int x1 = (int)(start.x + 0.5f);
    int y1 = (int)(start.y + 0.5f);
    ...
    if ((dx != 0) && (abs(dy/dx) < 1))
    {
        ...
        {
            RLImageDrawLine(dst, x1, y1 + i, x2, y2 + i, color);
        }
    ...
    else if (dy != 0)
    {
        ...
        {
            RLImageDrawLine(dst, x1 + i, y1, x2 + i, y2, color);
        }
...
void RLImageDrawCircle(RLImage *dst, int centerX, int centerY, int radius, RLColor color)
{
    int x = 0;
    int y = radius;
...
void RLImageDrawCircleV(RLImage *dst, RLVector2 center, int radius, RLColor color)
{
    RLImageDrawCircle(dst, (int)center.x, (int)center.y, radius, color);
}

// Draw circle outline within an image
void RLImageDrawCircleLines(RLImage *dst, int centerX, int centerY, int radius, RLColor color)
{
    int x = 0;
    int y = radius;
    int decesionParameter = 3 - 2*radius;
...
void RLImageDrawCircleLinesV(RLImage *dst, RLVector2 center, int radius, RLColor color)
{
    RLImageDrawCircleLines(dst, (int)center.x, (int)center.y, radius, color);
}

// Draw rectangle within an image
void RLImageDrawRectangle(RLImage *dst, int posX, int posY, int width, int height, RLColor color)
{
    RLImageDrawRectangleRec(dst, (RLRectangle){ (float)posX, (float)posY, (float)width, (float)height }, color);
}

// Draw rectangle within an image (Vector version)
void RLImageDrawRectangleV(RLImage *dst, RLVector2 position, RLVector2 size, RLColor color)
{
    RLImageDrawRectangle(dst, (int)position.x, (int)position.y, (int)size.x, (int)size.y, color);
}

// Draw rectangle within an image
void RLImageDrawRectangleRec(RLImage *dst, RLRectangle rec, RLColor color)
{
    // Security check to avoid program crash
    if ((dst->data == NULL) || (dst->width == 0) || (dst->height == 0)) return;
...
void RLImageDrawRectangleLines(RLImage *dst, RLRectangle rec, int thick, RLColor color)
{
...
void RLImageDrawTriangle(RLImage *dst, RLVector2 v1, RLVector2 v2, RLVector2 v3, RLColor color)
{
...
void RLImageDrawTriangleEx(RLImage *dst, RLVector2 v1, RLVector2 v2, RLVector2 v3, RLColor c1, RLColor c2, RLColor c3)
{
    // Calculate the 2D bounding box of the triangle
    // Determine the minimum and maximum x and y coordinates of the triangle vertices
    int xMin = (int)((v1.x < v2.x)? ((v1.x < v3.x)? v1.x : v3.x) : ((v2.x < v3.x)? v2.x : v3.x));
...
void RLImageDrawTriangleLines(RLImage *dst, RLVector2 v1, RLVector2 v2, RLVector2 v3, RLColor color)
{
...
void RLImageDrawTriangleFan(RLImage *dst, const RLVector2 *points, int pointCount, RLColor color)
{
    if (pointCount >= 3)
    {
...
void RLImageDrawTriangleStrip(RLImage *dst, const RLVector2 *points, int pointCount, RLColor color)
{
    if (pointCount >= 3)
    {
...
void RLImageDraw(RLImage *dst, RLImage src, RLRectangle srcRec, RLRectangle dstRec, RLColor tint)
{
    // Security check to avoid program crash
    if ((dst->data == NULL) || (dst->width == 0) || (dst->height == 0) ||
        (src.data == NULL) || (src.width == 0) || (src.height == 0)) return;

    if (dst->format >= RL_E_PIXELFORMAT_COMPRESSED_DXT1_RGB) TRACELOG(RL_E_LOG_WARNING, "Image drawing not supported for compressed formats");
    else
    {
        ...
        if ((tint.a == 255) &&
            ((srcPtr->format == RL_E_PIXELFORMAT_UNCOMPRESSED_GRAYSCALE) ||
            (srcPtr->format == RL_E_PIXELFORMAT_UNCOMPRESSED_R5G6B5) ||
            (srcPtr->format == RL_E_PIXELFORMAT_UNCOMPRESSED_R8G8B8) ||
            (srcPtr->format == RL_E_PIXELFORMAT_UNCOMPRESSED_R32) ||
            (srcPtr->format == RL_E_PIXELFORMAT_UNCOMPRESSED_R32G32B32) ||
            (srcPtr->format == RL_E_PIXELFORMAT_UNCOMPRESSED_R16) ||
            (srcPtr->format == RL_E_PIXELFORMAT_UNCOMPRESSED_R16G16B16)))
            blendRequired = false;
...
void RLImageDrawText(RLImage *dst, const char *text, int posX, int posY, int fontSize, RLColor color)
{
#if defined(SUPPORT_MODULE_RTEXT) && defined(SUPPORT_DEFAULT_FONT)
    // Make sure default font is loaded to be used on image text drawing
    if (RLGetFontDefault().texture.id == 0) LoadFontDefault();
...
#else
    TRACELOG(RL_E_LOG_WARNING, "IMAGE: ImageDrawText() requires module: rtext");
#endif
}

// Draw text (custom sprite font) within an image (destination)
void RLImageDrawTextEx(RLImage *dst, RLFont font, const char *text, RLVector2 position, float fontSize, float spacing, RLColor tint)
{
    RLImage imText = RLImageTextEx(font, text, fontSize, spacing, tint);
...
RLTexture2D RLLoadTexture(const char *fileName)
{
    RLTexture2D texture = { 0 };

    RLImage image = RLLoadImage(fileName);
...
RLTexture2D RLLoadTextureFromImage(RLImage image)
{
    RLTexture2D texture = { 0 };
    ...
    else TRACELOG(RL_E_LOG_WARNING, "IMAGE: Data is not valid to load texture");
...
RLTextureCubemap RLLoadTextureCubemap(RLImage image, int layout)
{
    RLTextureCubemap cubemap = { 0 };

    if (layout == RL_E_CUBEMAP_LAYOUT_AUTO_DETECT)      // Try to automatically guess layout type
    {
        // Check image width/height to determine the type of cubemap provided
        if (image.width > image.height)
        {
            if ((image.width/6) == image.height) { layout = RL_E_CUBEMAP_LAYOUT_LINE_HORIZONTAL; cubemap.width = image.width/6; }
            else if ((image.width/4) == (image.height/3)) { layout = RL_E_CUBEMAP_LAYOUT_CROSS_FOUR_BY_THREE; cubemap.width = image.width/4; }
        }
        else if (image.height > image.width)
        {
            if ((image.height/6) == image.width) { layout = RL_E_CUBEMAP_LAYOUT_LINE_VERTICAL; cubemap.width = image.height/6; }
            else if ((image.width/3) == (image.height/4)) { layout = RL_E_CUBEMAP_LAYOUT_CROSS_THREE_BY_FOUR; cubemap.width = image.width/3; }
        }
    }
    else
    {
        if (layout == RL_E_CUBEMAP_LAYOUT_LINE_VERTICAL) cubemap.width = image.height/6;
        if (layout == RL_E_CUBEMAP_LAYOUT_LINE_HORIZONTAL) cubemap.width = image.width/6;
        if (layout == RL_E_CUBEMAP_LAYOUT_CROSS_THREE_BY_FOUR) cubemap.width = image.width/3;
        if (layout == RL_E_CUBEMAP_LAYOUT_CROSS_FOUR_BY_THREE) cubemap.width = image.width/4;
    }

    cubemap.height = cubemap.width;

    // Layout provided or already auto-detected
    if (layout != RL_E_CUBEMAP_LAYOUT_AUTO_DETECT)
    {
        ...
        if (layout == RL_E_CUBEMAP_LAYOUT_LINE_VERTICAL)
        {
        ...
        {
            if (layout == RL_E_CUBEMAP_LAYOUT_LINE_HORIZONTAL) for (int i = 0; i < 6; i++) faceRecs[i].x = (float)size*i;
            else if (layout == RL_E_CUBEMAP_LAYOUT_CROSS_THREE_BY_FOUR)
            {
            ...
            else if (layout == RL_E_CUBEMAP_LAYOUT_CROSS_FOUR_BY_THREE)
            {
            ...
            RLImage mipmapped = RLImageCopy(image);
        #if defined(SUPPORT_IMAGE_MANIPULATION)
            if (image.mipmaps > 1)
            {
            ...
            for (int i = 0; i < 6; i++) RLImageDraw(&faces, mipmapped, faceRecs[i], (RLRectangle){ 0, (float)size*i, (float)size, (float)size }, WHITE);
    ...
    else TRACELOG(RL_E_LOG_WARNING, "IMAGE: Failed to detect cubemap image layout");
...
RLRenderTexture2D RLLoadRenderTexture(int width, int height)
{
    ...
    if (target.id > 0)
    {
        ...
        target.texture.id = rlLoadTexture(NULL, width, height, RL_E_PIXELFORMAT_UNCOMPRESSED_R8G8B8A8, 1);
...
bool RLIsTextureValid(RLTexture2D texture)
{
    bool result = false;
...
void RLUpdateTexture(RLTexture2D texture, const void *pixels)
{
...
void RLUpdateTextureRec(RLTexture2D texture, RLRectangle rec, const void *pixels)
{
...
void RLGenTextureMipmaps(RLTexture2D *texture)
{
...
void RLSetTextureFilter(RLTexture2D texture, int filter)
{
    switch (filter)
    {
        case RL_E_TEXTURE_FILTER_POINT:
        {
        ...
        case RL_E_TEXTURE_FILTER_BILINEAR:
        {
            if (texture.mipmaps > 1)
            {
        ...
        case RL_E_TEXTURE_FILTER_TRILINEAR:
        {
            if (texture.mipmaps > 1)
            {
            ...
            {
                TRACELOG(RL_E_LOG_WARNING, "TEXTURE: [ID %i] No mipmaps available for TRILINEAR texture filtering", texture.id);
        ...
        case RL_E_TEXTURE_FILTER_ANISOTROPIC_4X: rlTextureParameters(texture.id, RL_TEXTURE_FILTER_ANISOTROPIC, 4); break;
        case RL_E_TEXTURE_FILTER_ANISOTROPIC_8X: rlTextureParameters(texture.id, RL_TEXTURE_FILTER_ANISOTROPIC, 8); break;
        case RL_E_TEXTURE_FILTER_ANISOTROPIC_16X: rlTextureParameters(texture.id, RL_TEXTURE_FILTER_ANISOTROPIC, 16); break;
        default: break;
    }
}

// Set texture wrapping mode
void RLSetTextureWrap(RLTexture2D texture, int wrap)
{
    switch (wrap)
    {
        case RL_E_TEXTURE_WRAP_REPEAT:
        {
        ...
        case RL_E_TEXTURE_WRAP_MIRROR_REPEAT:
        {
        ...
        case RL_E_TEXTURE_WRAP_MIRROR_CLAMP:
        {
...
//------------------------------------------------------------------------------------
// Draw a texture
void RLDrawTexture(RLTexture2D texture, int posX, int posY, RLColor tint)
{
    RLDrawTextureEx(texture, (RLVector2){ (float)posX, (float)posY }, 0.0f, 1.0f, tint);
}

// Draw a texture with position defined as Vector2
void RLDrawTextureV(RLTexture2D texture, RLVector2 position, RLColor tint)
{
    RLDrawTextureEx(texture, position, 0, 1.0f, tint);
}

// Draw a texture with extended parameters
void RLDrawTextureEx(RLTexture2D texture, RLVector2 position, float rotation, float scale, RLColor tint)
{
...
void RLDrawTextureRec(RLTexture2D texture, RLRectangle source, RLVector2 position, RLColor tint)
{
...
void RLDrawTexturePro(RLTexture2D texture, RLRectangle source, RLRectangle dest, RLVector2 origin, float rotation, RLColor tint)
{
...
void RLDrawTextureNPatch(RLTexture2D texture, RLNPatchInfo nPatchInfo, RLRectangle dest, RLVector2 origin, float rotation, RLColor tint)
{
    if (texture.id > 0)
    {
        float width = (float)texture.width;
        ...
        if (nPatchInfo.layout == RL_E_NPATCH_THREE_PATCH_HORIZONTAL) patchHeight = nPatchInfo.source.height;
        if (nPatchInfo.layout == RL_E_NPATCH_THREE_PATCH_VERTICAL) patchWidth = nPatchInfo.source.width;
        ...
        if (patchWidth <= (leftBorder + rightBorder) && nPatchInfo.layout != RL_E_NPATCH_THREE_PATCH_VERTICAL)
        {
            drawCenter = false;
        ...
        if (patchHeight <= (topBorder + bottomBorder) && nPatchInfo.layout != RL_E_NPATCH_THREE_PATCH_HORIZONTAL)
        {
            drawMiddle = false;
        ...
        rlPushMatrix();
            ...
            rlBegin(RL_QUADS);
                ...
                if (nPatchInfo.layout == RL_E_NPATCH_NINE_PATCH)
                {
                    // ------------------------------------------------------------
                    // TOP-LEFT QUAD
                ...
                else if (nPatchInfo.layout == RL_E_NPATCH_THREE_PATCH_VERTICAL)
                {
                    // TOP QUAD
                    // -----------------------------------------------------------
                ...
                else if (nPatchInfo.layout == RL_E_NPATCH_THREE_PATCH_HORIZONTAL)
                {
                    // LEFT QUAD
                    // -----------------------------------------------------------
                    // Texture coords                 Vertices
...
bool RLColorIsEqual(RLColor col1, RLColor col2)
{
    bool result = false;
...
RLColor RLFade(RLColor color, float alpha)
{
...
int RLColorToInt(RLColor color)
{
    int result = 0;
...
RLVector4 RLColorNormalize(RLColor color)
{
    RLVector4 result;
...
RLColor RLColorFromNormalized(RLVector4 normalized)
{
    RLColor result;
...
RLVector3 RLColorToHSV(RLColor color)
{
    RLVector3 hsv = { 0 };
...
RLColor RLColorFromHSV(float hue, float saturation, float value)
{
...
RLColor RLColorTint(RLColor color, RLColor tint)
{
    RLColor result = color;
...
RLColor RLColorBrightness(RLColor color, float factor)
{
    RLColor result = color;
...
RLColor RLColorContrast(RLColor color, float contrast)
{
    RLColor result = color;
...
// Get color with alpha applied, alpha goes from 0.0f to 1.0f
RLColor RLColorAlpha(RLColor color, float alpha)
{
    RLColor result = color;
...
RLColor RLColorAlphaBlend(RLColor dst, RLColor src, RLColor tint)
{
    RLColor out = WHITE;
...
#define COLORALPHABLEND_INTEGERS
    ...
    {
        RLVector4 fdst = RLColorNormalize(dst);
        RLVector4 fsrc = RLColorNormalize(src);
        RLVector4 ftint = RLColorNormalize(tint);
        RLVector4 fout = { 0 };
...
RLColor RLColorLerp(RLColor color1, RLColor color2, float factor)
{
    RLColor color = { 0 };
...
RLColor RLGetColor(unsigned int hexValue)
{
    RLColor color;
...
RLColor RLGetPixelColor(void *srcPtr, int format)
{
    RLColor color = { 0 };

    switch (format)
    {
        case RL_E_PIXELFORMAT_UNCOMPRESSED_GRAYSCALE: color = (RLColor){ ((unsigned char *)srcPtr)[0], ((unsigned char *)srcPtr)[0], ((unsigned char *)srcPtr)[0], 255 }; break;
        case RL_E_PIXELFORMAT_UNCOMPRESSED_GRAY_ALPHA: color = (RLColor){ ((unsigned char *)srcPtr)[0], ((unsigned char *)srcPtr)[0], ((unsigned char *)srcPtr)[0], ((unsigned char *)srcPtr)[1] }; break;
        case RL_E_PIXELFORMAT_UNCOMPRESSED_R5G6B5:
        {
        ...
        case RL_E_PIXELFORMAT_UNCOMPRESSED_R8G8B8A8: color = (RLColor){ ((unsigned char *)srcPtr)[0], ((unsigned char *)srcPtr)[1], ((unsigned char *)srcPtr)[2], ((unsigned char *)srcPtr)[3] }; break;
        case RL_E_PIXELFORMAT_UNCOMPRESSED_R8G8B8: color = (RLColor){ ((unsigned char *)srcPtr)[0], ((unsigned char *)srcPtr)[1], ((unsigned char *)srcPtr)[2], 255 }; break;
        case RL_E_PIXELFORMAT_UNCOMPRESSED_R32:
        {
...
void RLSetPixelColor(void *dstPtr, RLColor color, int format)
{
    switch (format)
    {
        case RL_E_PIXELFORMAT_UNCOMPRESSED_GRAYSCALE:
...
int RLGetPixelDataSize(int width, int height, int format)
{
    ...
    switch (format)
    {
        case RL_E_PIXELFORMAT_UNCOMPRESSED_GRAYSCALE: bpp = 8; break;
        case RL_E_PIXELFORMAT_UNCOMPRESSED_GRAY_ALPHA:
        case RL_E_PIXELFORMAT_UNCOMPRESSED_R5G6B5:
        case RL_E_PIXELFORMAT_UNCOMPRESSED_R5G5B5A1:
        case RL_E_PIXELFORMAT_UNCOMPRESSED_R4G4B4A4: bpp = 16; break;
        case RL_E_PIXELFORMAT_UNCOMPRESSED_R8G8B8A8: bpp = 32; break;
        case RL_E_PIXELFORMAT_UNCOMPRESSED_R8G8B8: bpp = 24; break;
        case RL_E_PIXELFORMAT_UNCOMPRESSED_R32: bpp = 32; break;
        case RL_E_PIXELFORMAT_UNCOMPRESSED_R32G32B32: bpp = 32*3; break;
        case RL_E_PIXELFORMAT_UNCOMPRESSED_R32G32B32A32: bpp = 32*4; break;
        case RL_E_PIXELFORMAT_UNCOMPRESSED_R16: bpp = 16; break;
        case RL_E_PIXELFORMAT_UNCOMPRESSED_R16G16B16: bpp = 16*3; break;
        case RL_E_PIXELFORMAT_UNCOMPRESSED_R16G16B16A16: bpp = 16*4; break;
        case RL_E_PIXELFORMAT_COMPRESSED_DXT1_RGB:
        case RL_E_PIXELFORMAT_COMPRESSED_DXT1_RGBA:
        case RL_E_PIXELFORMAT_COMPRESSED_ETC1_RGB:
        case RL_E_PIXELFORMAT_COMPRESSED_ETC2_RGB:
        case RL_E_PIXELFORMAT_COMPRESSED_PVRT_RGB:
        case RL_E_PIXELFORMAT_COMPRESSED_PVRT_RGBA: bpp = 4; break;
        case RL_E_PIXELFORMAT_COMPRESSED_DXT3_RGBA:
        case RL_E_PIXELFORMAT_COMPRESSED_DXT5_RGBA:
        case RL_E_PIXELFORMAT_COMPRESSED_ETC2_EAC_RGBA:
        case RL_E_PIXELFORMAT_COMPRESSED_ASTC_4x4_RGBA: bpp = 8; break;
        case RL_E_PIXELFORMAT_COMPRESSED_ASTC_8x8_RGBA: bpp = 2; break;
        default: break;
    }
    ...
    if ((width < 4) && (height < 4))
    {
        if ((format >= RL_E_PIXELFORMAT_COMPRESSED_DXT1_RGB) && (format < RL_E_PIXELFORMAT_COMPRESSED_DXT3_RGBA)) dataSize = 8;
        else if ((format >= RL_E_PIXELFORMAT_COMPRESSED_DXT3_RGBA) && (format < RL_E_PIXELFORMAT_COMPRESSED_ASTC_8x8_RGBA)) dataSize = 16;
    }
