...
unsigned char *Win32GetClipboardImageData(int *width, int *height, unsigned long long int *dataSize)
{
    ...
    if (OpenClipboardRetrying(NULL))
    {
        ...
        if (clipHandle != NULL)
        {
                ...
                *height = bmpInfoHeader->biHeight;
                ...
                if (clipDataSize >= sizeof(BITMAPINFOHEADER))
                {
                    ...
                    *dataSize = bmpFileSize;
                    ...
                    TRACELOG(RL_E_LOG_INFO, "Clipboad image acquired successfully");
                    //------------------------------------------------------------------------
                }
                else
                {
                    TRACELOG(RL_E_LOG_WARNING, "Clipboard data is malformed");
                    GlobalUnlock(clipHandle);
                    CloseClipboard();
                }
            }
            ...
            {
                TRACELOG(RL_E_LOG_WARNING, "Clipboard data failed to be locked");
                GlobalUnlock(clipHandle);
                CloseClipboard();
            }
        }
        ...
        {
            TRACELOG(RL_E_LOG_WARNING, "Clipboard data is not an image");
            CloseClipboard();
        }
    }
    else TRACELOG(RL_E_LOG_WARNING, "Clipboard can not be opened");
