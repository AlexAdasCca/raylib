...
Light CreateLight(int type, RLVector3 position, RLVector3 target, RLColor color, RLShader shader)
{
    ...
    if (lightsCount < MAX_LIGHTS)
    {
        ...
        light.enabledLoc = RLGetShaderLocation(shader, RLTextFormat("lights[%i].enabled", lightsCount));
        light.typeLoc = RLGetShaderLocation(shader, RLTextFormat("lights[%i].type", lightsCount));
        light.positionLoc = RLGetShaderLocation(shader, RLTextFormat("lights[%i].position", lightsCount));
        light.targetLoc = RLGetShaderLocation(shader, RLTextFormat("lights[%i].target", lightsCount));
        light.colorLoc = RLGetShaderLocation(shader, RLTextFormat("lights[%i].color", lightsCount));
...
void UpdateLightValues(RLShader shader, Light light)
{
    ...
    RLSetShaderValue(shader, light.colorLoc, color, RL_E_SHADER_UNIFORM_VEC4);
}
