...
int main(void)
{
    ...
    RLInitWindow(screenWidth, screenHeight, "raylib [models] example - tesseract view");
    ...
    while (!RLWindowShouldClose())    // Detect window close button or ESC key
    {
        // Update
        //----------------------------------------------------------------------------------
        rotation = DEG2RAD*45.0f*(float)RLGetTime();
        ...
        for (int i = 0; i < 16; i++)
        {
            ...
            transformed[i] = (RLVector3){ p.x, p.y, p.z };
            wValues[i] = p.w;
        }
        //----------------------------------------------------------------------------------
        ...
        RLBeginDrawing();
            ...
            RLBeginMode3D(camera);
                for (int i = 0; i < 16; i++)
                {
                    ...
                    RLDrawSphere(transformed[i], fabsf(wValues[i]*0.1f), RED);

                    for (int j = 0; j < 16; j++)
                    {
                        // Two lines are connected if they differ by 1 coordinate
                        // This way we dont have to keep an edge list
                        RLVector4 v1 = tesseract[i];
                        RLVector4 v2 = tesseract[j];
                        ...
                        if (diff == 3 && i < j) RLDrawLine3D(transformed[i], transformed[j], MAROON);
                    }
                }
            RLEndMode3D();
