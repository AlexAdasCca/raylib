...
int main(void)
{
    ...
    RLInitWindow(screenWidth, screenHeight, "raylib [textures] example - gif player");
    ...
    RLTexture2D texScarfyAnim = RLLoadTextureFromImage(imScarfyAnim);
    ...
    while (!RLWindowShouldClose())    // Detect window close button or ESC key
    {
        ...
        if (frameCounter >= frameDelay)
        {
            ...
            RLUpdateTexture(texScarfyAnim, ((unsigned char *)imScarfyAnim.data) + nextFrameDataOffset);
    ...
    // De-Initialization
    //--------------------------------------------------------------------------------------
    RLUnloadTexture(texScarfyAnim);   // Unload texture
