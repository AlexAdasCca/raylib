...
int main(void)
{
    ...
    RLVector2 padPosition = { 100, 350 };
    float buttonRadius = 30;

    RLVector2 buttonPositions[BUTTON_MAX] = {
    ...
    RLColor buttonLabelColors[BUTTON_MAX] = {
        YELLOW, // Up
        BLUE,   // Left
        RED,    // Right
        GREEN   // Down
    ...
    RLVector2 inputPosition = { 0, 0 };

    RLVector2 playerPosition = { (float)screenWidth/2, (float)screenHeight/2 };
    ...
    while (!RLWindowShouldClose())    // Detect window close button or ESC key
    {
        // Update
        //--------------------------------------------------------------------------
        if ((RLGetTouchPointCount() > 0)) inputPosition = RLGetTouchPosition(0); // Use touch position
        else inputPosition = RLGetMousePosition(); // Use mouse position
        ...
        {
            case BUTTON_UP: playerPosition.y -= playerSpeed*RLGetFrameTime(); break;
            case BUTTON_LEFT: playerPosition.x -= playerSpeed*RLGetFrameTime(); break;
            case BUTTON_RIGHT: playerPosition.x += playerSpeed*RLGetFrameTime(); break;
            case BUTTON_DOWN: playerPosition.y += playerSpeed*RLGetFrameTime(); break;
            default: break;
        };
        //--------------------------------------------------------------------------
        ...
        RLBeginDrawing();
            ...
            // Draw world
            RLDrawCircleV(playerPosition, 50, MAROON);

            // Draw GUI
            for (int i = 0; i < BUTTON_MAX; i++)
            {
                RLDrawCircleV(buttonPositions[i], buttonRadius, (i == pressedButton)? DARKGRAY : BLACK);
