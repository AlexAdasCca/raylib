...
int main(void)
{
    ...
    RLVector2 startingPositions[3] = {{ 400.0f, 150.0f }, { 300.0f, 300.0f }, { 500.0f, 300.0f }};
    RLVector2 trianglePositions[3] = { startingPositions[0], startingPositions[1], startingPositions[2] };
    ...
    while (!RLWindowShouldClose())    // Detect window close button or ESC key
    {
        ...
        if (triangleIndex != -1)
        {
            RLVector2 *position = &trianglePositions[triangleIndex];
        ...
        RLBeginDrawing();
            ...
            for (unsigned int i = 0; i < 3; i++)
            {
                ...
                if (RLCheckCollisionPointCircle(RLGetMousePosition(), trianglePositions[i], handleRadius))
                    RLDrawCircleV(trianglePositions[i], handleRadius, RLColorAlpha(DARKGRAY, 0.5f));

                // Draw handle fill selected
                if (i == triangleIndex) RLDrawCircleV(trianglePositions[i], handleRadius, DARKGRAY);

                // Draw handle outline
                RLDrawCircleLinesV(trianglePositions[i], handleRadius, BLACK);
            }

            // Draw controls
            RLDrawText("SPACE: Toggle lines mode", 10, 10, 20, DARKGRAY);
            RLDrawText("LEFT-RIGHT: Toggle backface culling", 10, 40, 20, DARKGRAY);
            RLDrawText("MOUSE: Click and drag vertex points", 10, 70, 20, DARKGRAY);
            RLDrawText("R: Reset triangle to start positions", 10, 100, 20, DARKGRAY);
